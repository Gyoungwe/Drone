...
- One authorization now really runs an example task to the end. Real-SDK simulations of the six example tasks (plus the Zotero branch) showed that after the single authorization card a user still had to answer a 推进剩余事项 dialog or type 继续 up to 7 times, confirm 5 `human_review` milestones and consent to every Zotero write, and that any task that ran a command could never reach 已完成. Fixes: (1) when the model ends a turn early with deliverables remaining, the host hands the task back under the same authorization at `agent_end` (`reserveHandoff`: only with new observed progress, nothing waiting for the user, no unknown effect, at most 16 times) instead of asking again; (2) stage checkpoints are no longer vetoed by sibling calls of a parallel batch that are still running (a batch of legitimate writes was misreported as 阶段步数用完); (3) task completion no longer requires every effect to be independently verified — command effects (`bash` / `powershell`) have no artifact to read back and stayed `returned` forever; the remaining summary now states how many such steps have only a return record; (4) a DOI-less `zotero_item` milestone is a one-write slot covered by the task authorization (`consent → { slot: true }`, listed only while unbound; per-task claim table for parallel batches), so papers written after close reading no longer trigger a 写入 Zotero 文献库？ card each; (5) the five `human_review` example milestones became deliverable files (`scope.md`, `interpretation.md`, `revision-notes.md`, `delivery-check.md`, `handoff-check.md`) and the example prompt asks the model to finish every milestone after the one authorization; (6) the handoff policy tells the model the authorization covers all deliverables. `example-tasks-one-authorization-sdk.test.mjs` now asserts exactly one authorization card and zero further asks, permission confirms, panel actions or nudges for all seven scenarios.
- `research_archive_source` can now archive a paper from its DOI (or PMCID / PMID) alone: the host resolves a **legitimate open-access** PDF through Europe PMC (identity + OA flags), the PMC Article Datasets on AWS (`pmc-oa-opendata`, the official replacement for the retired `oa.fcgi` service: per-version metadata with license and `pdf_url`, plain HTTPS, no CAPTCHA), the Europe PMC rendered PDF, Unpaywall (`DRONE_CONTACT_EMAIL` / `UNPAYWALL_EMAIL`), OpenAlex, Semantic Scholar and Crossref, tries the candidates in trust order, keeps only real PDF bytes, and records every source queried (`.pi/lib/open-access.mjs`). A given `url` that fails (HTML landing page, 404, login wall) falls back to the same resolution when a DOI is known. When no lawful copy exists the tool answers `status=no_open_access` with the sources queried, the publisher hand-off if one asked for a login, and a manual-import path that respects the user's own access rights (browser download → `.pi/browser-downloads` → `local_file` + `human_verified=true`); it never touches Sci-Hub or other pirate mirrors, and the skill docs say so explicitly.

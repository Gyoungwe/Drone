...
export function createEvidenceGate({ scope = "factual-answer" } = {}) {
  ...
  const snapshot = () => ({ scope, stage, status: "ok", answerable: stage === "answerable", events: [...events] });
...
export function assertEvidenceAnswerable(gate, { claimRefs = [] } = {}) {
  ...
  if (state.stage !== "answerable" || !Array.isArray(claimRefs) || claimRefs.length === 0) {
    throw new Error("Evidence gate is closed: complete retrieval, inspection, archiving and claim binding first");
  }
  return state;
}

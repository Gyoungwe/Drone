...
export async function saveKnowledgeBinding(input, expectedRevision = null) {
	...
	const operation = previous
		...
		.then(async () => {
			...
			if (
				...
				throw new Error("Invalid knowledge policy");
	...
	try {
		...
		if (state.queues.get(dir) === operation) state.queues.delete(dir);
	}
}

...
const contained=(root,path)=>{const rel=relative(root,path);return !isAbsolute(rel)&&rel!=='..'&&!rel.startsWith('..'+sep);};
/** A generated document is stored, never executed or opened automatically. */
export function validateSpecialistHtml(html){
 if(typeof html!=='string'||html.length>64000||!/<html\b/i.test(html)||!/<body\b/i.test(html))throw new Error('Explainer must be one bounded HTML document');
 if(/<(?:script|iframe|frame|object|embed|base|form|link)\b|\bon[a-z]+\s*=|<meta\b[^>]*http-equiv\s*=|javascript\s*:|@import|url\s*\(/i.test(html))throw new Error('Explainer must be static and self-contained; active content was refused');
 if(/\bsrc\s*=\s*["']?(?!data:image\/)[^\s>]/i.test(html))throw new Error('Explainer external resources are not allowed');
 return html;
}
export async function saveSpecialistExplainer(c,ctx,{runDir,topicId,answer}){
 ...
 return withKnowledgeBinding(c.binding,async()=>{
  ...
  await mkdir(run,{recursive:true});await writeFile(artifactPath,html,{flag:'wx',mode:0o600});
  const archived=await publishExplainer({cwd:ctx.cwd,project:c.project,topicId,title:answer.data.title,artifactPath,summary:answer.data.summary,sources:sources.sources});
  return {...archived,result_file:artifactPath,producer:'knowledge-explainer',skill:'show-me'};
 });
}

...
const OMIT = new Set(['node_modules','dist','build','out','coverage','vendor','venv','__pycache__','Attachments','Templates','_template']);
...
export function canRead(path, project) { const scope = noteScope(path); return scope === 'shared' || scope === project; }
export function fileVersion(stat) { return `${stat.mtimeMs}:${stat.ctimeMs}:${stat.size}:${stat.ino}`; }
export async function safeNotePath(vault, path) {
  ...
  const actual = await realpath(full), rel = relative(vault, actual);
  if (!rel || isAbsolute(rel) || rel === '..' || rel.startsWith('..' + sep)) throw new Error('Note is outside the bound Vault');
  return actual;
}
export async function inspectNote(vault, path) {
  const full = await safeNotePath(vault, path), stat = await lstat(full);
...
export async function readNoteFile(vault, path) {
  ...
  try {
    ...
    if (!before.isFile() || before.size > MAX_NOTE_BYTES) throw new Error('Note exceeds the 1 MiB indexing limit');
    ...
    return { path, text: bytes.toString('utf8'), hash: createHash('sha256').update(bytes).digest('hex'), signature: fileVersion(after), bytes: offset };
  } finally { await handle.close(); }
}
export function snippet(text, { startLine = 1, maxChars = 5000 } = {}) {
  const lines = text.split('\n');
  const start = Math.max(0, Math.floor(startLine) - 1), selected = [];
  let chars = 0;
  for (let i = start; i < lines.length; i++) {
    ...
    selected.push(lines[i]); chars += lines[i].length + 1;
  }
  return { text: selected.join('\n'), startLine: start + 1, endLine: start + selected.length,
    totalLines: lines.length, truncated: start > 0 || start + selected.length < lines.length || (selected.length === 1 && selected[0].length < (lines[start]?.length || 0)) };
}

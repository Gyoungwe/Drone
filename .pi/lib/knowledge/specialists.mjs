...
export function createKnowledgeSpecialists(pi, { getCurrent, readOnly = false }) {
	...
	async function run(
		ctx,
		role,
		{ task = goal, sourcePaths = [], summary = "", automatic = false, targetPath = null } = {},
	) {
		...
		if (generation !== epoch || getCurrent(ctx) !== c || parentSignal?.aborted)
			return { status: "cancelled", role, reason: "parent-turn-changed" };
		if (readOnly || !ctx.isProjectTrusted?.() || c.binding.subagentPolicy !== "read-local")
		...
		const [name, label] = profiles[role],
			id = randomUUID();
		let progress = { id, role, name, label, status: "queued", startedAt: Date.now() };
		const publish = (patch) => {
		...
		try {
			const answer = await withSpecialistSlot(controller.signal, async () => {
				...
				if (targetPath) {
					...
					if (!canRead(targetPath, c.project)) throw new Error("Wiki target is outside the permitted scope");
					allowed.add(targetPath);
				}
				const read = async ({ path, start_line = 1 }) => {
					await check();
					if (++readCount > 8) throw new Error("Specialist read budget exhausted");
					validateNote(path);
				...
				if (role === "navigator")
					capabilities.push({
						...
						execute: async ({ query, wiki_only = false }) => {
							await check();
							if (++searchCount > 2) throw new Error("Specialist search budget exhausted");
	...
	function compact(result) {
		if (result.status !== "completed")
			return { role: result.role, status: result.status, reason: result.reason };
		return {
			role: result.role,
	...
	async function orient(ctx) {
		...
		if (
			...
			const paths = nav.refs
				...
				.slice(0, 4);
			if (paths.length >= 2) {
				const evidence = await run(ctx, "evidence", { sourcePaths: paths, automatic: true });

...
import { readNoteFile } from "./files.mjs";
...
export async function updateNavigation(vault, path, heading, body) {
	if (
		!["Home.md", "Wiki/Index.md", "Library/Index.md"].includes(path) &&
		!/^Projects\/[a-z0-9-]+\/Index\.md$/.test(path)
	)
	...
	const start = base.indexOf(START),
		end = base.indexOf(END),
		block = `${START}\n${body.trim()}\n${END}`;
	if (
		start < 0 !== end < 0 ||
		end < start ||
		(start >= 0 && (base.indexOf(START, start + 1) >= 0 || base.indexOf(END, end + 1) >= 0))
	)
		throw new Error("Invalid navigation managed markers");
	const updated =
		start >= 0
			? base.slice(0, start) + block + base.slice(end + END.length)
			: `${base.trimEnd()}\n\n${block}\n`;
	...
	if (original === null) {
		await writeFile(full, updated, { flag: "wx" });
...
export async function runNavigationMaintenance(service, project, limit = 3) {
	const jobs = await service.request("jobs", { project, kind: "navigation" }),
		out = [];
	for (const item of jobs.items.filter((job) => job.kind === "navigation").slice(0, Math.min(10, limit))) {
		...
		try {
			...
			out.push({ path: item.path, error: error.message });
		}
	}
	return {
		navigation: out,

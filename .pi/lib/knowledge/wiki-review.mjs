		...
		if ((await readReviewMode()) === "automatic") {
			try {
				return await decideWikiProposal(service, proposal.id, project, proposal.proposalHash, "apply",
					{ actor: "automatic", authority: AUTOMATIC_AUTHORITY });
			} catch {
		...
		withKnowledgeBinding(service.binding, async () => {
			...
			return exclusive(`${service.binding.vault}:wiki:${p.path}`, async () => {
				...
				if (review.actor === "automatic") {
					if ((await readReviewMode()) !== "automatic") throw new Error("Strict review requires confirmation");
...
export async function wikiHistory(service, project) {
 return withKnowledgeBinding(service.binding, async () => {
  let names;
  try { names = await readdir(join(rootFor(service), "reviewed")); }
  catch (error) { if (error.code === "ENOENT") return []; throw error; }
  const items = [];
  for (const name of names.filter(n => /^[0-9a-f-]{36}\.json$/.test(n))) {
   let p;
   try { p = await loadProposal(service, name.slice(0, -5), project); } catch { continue; }
   if (p.status !== "applied") continue;
   items.push({ id: p.id, path: p.path, afterHash: p.afterHash, reviewedAt: p.reviewedAt, reviewMethod: p.reviewMethod });
  }
...
export async function undoWikiUpdate(service, project, id, expectedHash) {
 ...
 return exclusive(`${rootFor(service)}:${id}`, () => withKnowledgeBinding(service.binding, async () => {
  const p = await loadProposal(service, id, project);
  if (p.status !== "applied" || p.afterHash !== expectedHash) throw new Error("Refresh Wiki history before undo");
  return exclusive(`${service.binding.vault}:wiki:${p.path}`, async () => {
   const current = await currentNote(service, p.path);
   if (current?.hash !== p.afterHash) throw new Error("Wiki changed since saving; undo would overwrite edits");
   const full = join(service.binding.vault, p.path);
   if (p.before === null) await unlink(full);
    ...
    try {
     ...
     if ((await currentNote(service, p.path))?.hash !== p.afterHash) throw new Error("Wiki changed during undo");

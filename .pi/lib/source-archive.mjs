...
function safeFilename(input, fallback) {
  ...
  const candidate = basename(value).replace(/[<>:"/\\|?*\u0000-\u001f]/g, "-").replace(/\s+/g, " ").trim();
...
function filenameFromResponse(response, url, category) {
  ...
  if (!filename) {
    try { filename = basename(new URL(url).pathname); } catch { /* fallback below */ }
  }
  const fallback = `${category}-${new Date().toISOString().replace(/[-:.TZ]/g, "").slice(0, 14)}.bin`;
...
function looksLikeChallenge(status, contentType, sample) {
  ...
  return /cf-chl|cloudflare|captcha|verify you are human|access denied|sign in|log in|login required/i.test(sample);
}

function hasMagic(bytes) {
  if (bytes.length >= 5 && new TextDecoder().decode(bytes.subarray(0, 5)) === "%PDF-") return true;
  if (bytes.length >= 4 && bytes[0] === 0x50 && bytes[1] === 0x4b && (bytes[2] === 0x03 || bytes[2] === 0x05 || bytes[2] === 0x07)) return true;
...
function validateContent(category, contentType, filename, bytes) {
  ...
  if (category === "software" && mime === "text/html") return { ok: false, reason: "HTML is not a software archive" };
...
async function withRunLock(key, fn) {
  ...
  try { return await current; } finally { if (RUN_LOCKS.get(key) === current) RUN_LOCKS.delete(key); }
}

async function readManifest(file, runDir) {
  try {
    const parsed = JSON.parse(await readFile(file, "utf8"));
    if (!parsed || typeof parsed !== "object" || !Array.isArray(parsed.items)) throw new Error("invalid manifest");
    return parsed;
  } catch (error) {
...
async function readBody(response, maxBytes) {
  ...
  if (Number.isFinite(declared) && declared > maxBytes) throw new Error(`response exceeds max_bytes (${maxBytes})`);
  if (!response.body) {
...
function normaliseMetadata(metadata = {}) {
  ...
  return Object.fromEntries(allowed.filter((key) => metadata[key] != null && String(metadata[key]).trim() !== "").map((key) => [key, metadata[key]]));
}

export async function archiveSource({ cwd = process.cwd(), run_dir, url, category = "papers", filename, metadata, local_file, human_verified = false, content_type, max_bytes = DEFAULT_MAX_BYTES, timeout_ms = DEFAULT_TIMEOUT_MS, fetchImpl = globalThis.fetch } = {}) {
  if (!SOURCE_CATEGORIES.includes(category)) throw new Error(`category must be one of: ${SOURCE_CATEGORIES.join(", ")}`);
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 500 * 1024 * 1024) throw new Error("max_bytes must be between 1 and 524288000");
  if (!Number.isInteger(timeout) || timeout < 100 || timeout > 10 * 60 * 1000) throw new Error("timeout_ms must be between 100 and 600000");
  ...
  if (!isWithin(await realpath(runDir), await realpath(sourcesDir))) throw new Error("sources directory escapes the run through a link");
  const persist = async (bytes, { resolvedUrl, contentType, outputName, verified = false }) => withRunLock(manifestPath, async () => {
    ...
    try {
      ...
      if (createHash("sha256").update(existing).digest("hex") !== sha256) throw new Error("Archived version path has different content");
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    ...
    await atomicJson(manifestPath, manifest);
    let knowledge;
    try { knowledge = await publishSourceNote({ cwd, runDir, entry }); }
    catch (error) { knowledge = { obsidian_note: null, knowledge_status: "failed", obsidian_error: error.message }; }
    return { ...entry, ...knowledge, manifest_path: manifestPath, manifestPath, failures_path: await access(failuresPath).then(() => failuresPath, () => null) };
  });
  if (local_file != null) {
    try {
      ...
      return await persist(bytes, { resolvedUrl: url, contentType: content_type || metadata?.content_type || "application/octet-stream", outputName, verified: true });
    } catch (error) {
      const failure = { url, category, failed_at: new Date().toISOString(), reason: error.message, browser_required: false, local_file };
  ...
  let response;
  try {
    response = await fetchImpl(url, { redirect: "follow", signal: controller.signal, headers: { accept: "application/pdf,application/zip,application/octet-stream,text/plain,text/markdown,text/html;q=0.8,*/*;q=0.1" } });
    const contentType = response.headers.get("content-type") || "application/octet-stream";
    if (looksLikeChallenge(response.status, contentType, "")) {
      const failure = { url, category, failed_at: new Date().toISOString(), reason: `HTTP ${response.status} requires browser verification or login`, browser_required: true, browser_handoff: { url, action: "Open this URL in Computer Use browser, complete verification manually, then retry archive" } };
      await recordFailure(manifestPath, failuresPath, runDir, failure);
      return { status: "browser_required", browser_required: true, ...failure, manifest_path: manifestPath, failures_path: failuresPath };
    }
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const bytes = await readBody(response, limit);
    const resolvedUrl = response.url || url;
    const outputName = safeFilename(filename || filenameFromResponse(response, resolvedUrl, category), `${category}-${randomUUID()}.bin`);
    ...
    if (looksLikeChallenge(response.status, contentType, sample)) {
      const failure = { url, category, failed_at: new Date().toISOString(), reason: "Response requires browser verification or login", browser_required: true, browser_handoff: { url, action: "Open this URL in Computer Use browser, complete verification manually, then retry archive" } };
      await recordFailure(manifestPath, failuresPath, runDir, failure);
      return { status: "browser_required", browser_required: true, ...failure, manifest_path: manifestPath, failures_path: failuresPath };
    }
    ...
    const failure = { url, category, failed_at: new Date().toISOString(), reason: error.message, browser_required: browserRequired, ...(browserRequired ? { browser_handoff: { url, action: "Open this URL in Computer Use browser, complete verification manually, then retry archive" } } : {}) };
    await recordFailure(manifestPath, failuresPath, runDir, failure);
    return { status: "failed", ...failure, manifest_path: manifestPath, failures_path: failuresPath };
  } finally {
    clearTimeout(timer);
...
export async function sourceStatus({ cwd = process.cwd(), run_dir } = {}) {
  ...
  const manifest = await readManifest(manifestPath, runDir);
  let failures = "";
  try { failures = await readFile(failuresPath, "utf8"); } catch (error) { if (error.code !== "ENOENT") throw error; }
  const manifestExists = await access(manifestPath).then(() => true, () => false);
  const failuresExists = await access(failuresPath).then(() => true, () => false);
  return { run_dir: runDir, manifest_path: manifestExists ? manifestPath : null, failures_path: failuresExists ? failuresPath : null, manifest, failure_count: manifest.failures?.length ?? Math.max(0, (failures.match(/^## /gm) || []).length) };
}

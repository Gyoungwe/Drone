...
export async function validateVaultPath(input, excluded = []) {
  ...
  if (path === parse(path).root || /^[a-z]:[\\/]*$/i.test(input)) throw new Error('Vault cannot be a filesystem root');
  for (const other of excluded.filter(Boolean)) {
    const forbidden = await canonicalPath(resolve(other));
    if (containsPath(path, forbidden) || containsPath(forbidden, path)) throw new Error('Vault must be independent from workspace, results and product code');
  }
  try { if (!(await lstat(path)).isDirectory()) throw new Error('Vault must be a directory'); }
...
export async function containedFile(root, path) {
  ...
  if (!containsPath(root, target) || !containsPath(root, await canonicalPath(target))) throw new Error('Path must stay inside Vault');
  return target;
}
...
export function renderTemplate(template, values = {}) {
  ...
  if (template.includes('id: pi-<stable-id>')) return rendered.replace('id: pi-<stable-id>', 'id: pi-' + fields.uuid).replace('project: ""', 'project: ' + JSON.stringify(fields.project_slug || ''));
  return rendered;
}
...
export async function createOnly(path, content) {
  ...
  try { await link(temporary, path); return true; }
    ...
    if (!existing.isFile() || existing.isSymbolicLink()) throw new Error('Expected a regular Vault file: ' + path);
    return false;
...
export async function initializeVaultLayout(input, { excluded = [], project = null } = {}) {
  ...
  Object.assign(notes, {
    ...
    'Projects/Index.md': index('Projects'), 'Indexes/Projects.md': index('Projects'),
    'Indexes/Knowledge.md': index('Knowledge'), 'Library/Index.md': index('Library'),

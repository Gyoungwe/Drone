...
export function deliveryContract(prompt,{showMeAvailable=false}={}){
 ...
 const requested=manual||(/下载|沉淀|介绍|讲解|解释|download|explain|summariz/i.test(text)&&(paper||software));
 if(!requested)return null;
 return {kind:software&&paper?'mixed':software?'software':'research',showMeAvailable,
  guidance:[
   ...
   showMeAvailable?'Use the loaded show-me skill for a focused explainer after reading the sources. Resolve and read its actual SKILL.md from the available skill catalog; save one HTML/Markdown explainer under this task run and link it in the visible final answer. Prefer the authorized knowledge-explainer host worker in automatic mode after summary save; do not launch a recursive model/run or duplicate its output. If handling the explanation directly, after creating it, archive it with research_archive_explainer using the active research run metadata.topic_id when available (reuse it across rounds) and the actual evidence notes read this turn; this produces a searchable Library/Explainers pointer plus a versioned attachment, but it remains presentation-only.':'A show-me skill is not loaded. Report this honestly; deliver the same explanation as Markdown rather than claiming the skill ran.',
...
export function assessManualPage(bytes,contentType,url){
 if(!/html/i.test(contentType||''))return {status:'document-unassessed',complete:false,candidates:[],note:'Content completeness and version still need inspection.'};
 ...
 for(const match of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){
  try{const resolved=new URL(match[1].replace(/&amp;/g,'&'),url),base=new URL(url);resolved.hash='';
   const title=match[2].replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim().slice(0,160);
   if(resolved.origin!==base.origin||!/^https?:$/.test(resolved.protocol)||resolved.username||resolved.password)continue;
   if(!/manual|option|command|parameter|usage|align|install|reference/i.test(title+' '+resolved.pathname)||resolved.href===base.href||candidates.some(c=>c.url===resolved.href))continue;
   candidates.push({url:resolved.href,title});if(candidates.length>=12)break;
  }catch{/* untrusted links */}
 }
 return {status:flags.length>=5?'reference-page-only':'landing-or-overview-page',complete:false,optionCount:flags.length,candidates,

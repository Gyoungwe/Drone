...
- Authorized dual deposition has two separate steps: Zotero metadata plus legally open fulltext when available (`research_archive_source category=papers doi=<doi>` resolves open-access PDFs through Europe PMC / PMC OA / Unpaywall / OpenAlex / Semantic Scholar / Crossref; `no_open_access` = abstract-only unless the user supplies the file through their own access); then an interpreted `Library/Papers` note with DOI and `zotero:<key>` links. `research_archive_source` alone does not import into Zotero.
- `zotero_item` milestones may leave `doi` empty at plan time: after `research_zotero_save` returns `saved`/`reused`, the host binds that DOI to the first DOI-less `zotero_item` milestone and verifies it by read-back. If a planned or bound DOI turns out to be the wrong paper, request `task_wait kind=rebind milestoneId=<id> doi=<real doi> reason=<why>`; only the user's confirmation applies it.

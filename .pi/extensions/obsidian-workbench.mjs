...
import { OBSIDIAN_SETUP_BINDING, inspectObsidianSetup, resolveSetupVault, setupAgentMessage } from '../lib/obsidian-setup.mjs';
...
async function startSetup(pi, args, ctx) {
  ...
  if (!pi.getCommands().some(command => command.source === 'skill' && command.name === skillCommand)) {
    throw new Error('Obsidian MCP setup requires the research-vault skill. Load the bundled skill and run /reload before retrying /obsidian-setup.');
  }
  const current = await obsidianStatus(ctx.cwd);
  let selectedPath = null;
  if (args?.trim().startsWith('{')) {
    try { const supplied=JSON.parse(args); if(typeof supplied.vaultPath==='string') {selectedPath=supplied.vaultPath;args='';} } catch { /* ordinary user preferences */ }
  }
  const input = selectedPath || await ctx.ui.input(
    'Obsidian · Vault 路径', current.vault || '/absolute/path/to/your/Obsidian Vault',
  );
  if (!input) return;
...
export default function obsidianWorkbench(pi) {
  ...
  pi.registerTool({
    ...
    description: 'Read-only setup discovery: return the actual session workspace and optional Vault directory overview, current settings, built-in profiles, deposition modes and subagent MCP policies. Does not write or read file contents.',
    parameters: { type: 'object', properties: { path: { type: 'string', description: 'Optional absolute target Vault path already supplied by the user.' } } },
    async execute(_id, params, _signal, _update, ctx) {
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ current, ...options }, null, 2) }], details: { current, ...options } };
    },
  });
  ...
  pi.registerTool({
    ...
    description: 'Initialize/bind an independent Obsidian vault, select a built-in knowledge profile, configure controlled deposition, and make research-obsidian retrieval-only. Parent session only. Reload after setup.',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string' },
    ...
    async execute(_id, params, signal, _update, ctx) {
      ...
      if (!profile || !options.depositModes.some(item => item.id === params.deposit_mode) || !options.subagentMcpPolicies.some(item => item.id === params.subagent_mcp)) throw new Error('Invalid setup options');
      const priorBinding = knowledgeDirectory() ? await readKnowledgeBinding({ fresh: true }) : null;
      const summary = [
        `实际项目：${ctx.cwd}`, `目标 Vault：${vault}`, `知识项目：${params.project || '暂不创建项目'}`,
        `模板：${profile.label}`, `项目目录：${profile.projectTypes.join(', ')}`,
        `共享 Library：${profile.libraryTypes.join(', ')}`,
        `知识沉淀：${params.deposit_mode}`, `子智能体 MCP：${params.subagent_mcp}`,
        knowledgeDirectory()
      ...
      const choice = await ctx.ui.select(`Obsidian · 确认应用初始化方案\n\n${summary}`, ['确认应用此方案', '取消，不做修改'], { signal });
      const confirmed = choice === '确认应用此方案';
      if (!confirmed || signal?.aborted) return { content: [{ type: 'text', text: 'Obsidian setup cancelled. No setup writes were performed.' }], details: { cancelled: true } };
  ...
  pi.registerTool({
    ...
    description: 'Create a project using the configured knowledge profile and update project indexes. Existing projects and human edits are preserved.',
    parameters: {
      type: 'object',
    ...
    async execute(_id, params, _signal, _update, ctx) {
      const result = await knowledge.withTurnBinding(ctx, () => createObsidianProject({ cwd: ctx.cwd, project: params.project, title: params.title || params.project }));
  ...
  pi.registerTool({
    ...
    parameters: {
      ...
      properties: {
        project: { type: 'string' },
        type: { type: 'string', enum: ['question', 'evidence', 'claim', 'decision', 'wiki', 'paper', 'method', 'software', 'entity', 'concept'] },
        title: { type: 'string' },
        markdown: { type: 'string' },
        source_links: { type: 'array', items: { type: 'string' } },
        status: { type: 'string' },
        scope: { type: 'string', enum: ['project', 'shared'], description: 'For Wiki only: shared publishes a cross-project topic.' },
      },
      required: ['project', 'type', 'title', 'markdown'],
    },
    async execute(_id, params, _signal, _update, ctx) {
      const result = await knowledge.withTurnBinding(ctx, () => depositKnowledge({
        cwd: ctx.cwd,
  ...
  for (const name of [setupCommand, ...aliases]) {
    pi.registerCommand(name, {
      ...
      perchoMenu: { version: 1, canonical: setupCommand, skill, role: name === setupCommand ? 'primary' : 'alias' },
      description: name === setupCommand
  ...
  pi.on('session_start', async (_event, ctx) => {
    ...
    if (status.state === 'ready') {
      if (ctx.hasUI) ctx.ui.notify(`Obsidian: ${status.vault} · ${status.profile || 'hybrid'} · ${status.scope === 'application' ? 'application indexed knowledge' : 'MCP retrieval-only'}`, 'info');
    } else {
      pi.sendMessage({
  ...
  pi.on('before_agent_start', async (event, ctx) => {
    ...
    return { ...(preparation.message ? { message: preparation.message } : {}), systemPrompt: `${event.systemPrompt}\n\n${guidance}\n${outputGuidance}\n${preparation.guidance || ""}` };
  });
}

...
export const OBSIDIAN_SUBAGENT_READ_TOOLS = [
  "read_note", "read_multiple_notes", "search_notes", "list_directory",
  "get_notes_info", "get_frontmatter", "get_vault_stats", "list_all_tags",
  "wiki_link", "get_note_outline", "read_note_lines"
];
  ...
  return async function subagentReadonlyMcp(pi) {
    if (knowledgeDirectory()) {
      ...
      pi.on('before_agent_start', async (event, ctx) => {
        ...
        return { ...(prepared.message ? { message: prepared.message } : {}), systemPrompt: `${event.systemPrompt}\n${prepared.guidance || ''}` };
      });
      return;
    }

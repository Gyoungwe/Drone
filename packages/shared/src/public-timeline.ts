...
export function publicTimeline<T extends { id: string; blockIndex?: number }>(input: {
	...
	thinking: string;
	tools: T[];
	steps: readonly PublicProgressStep[];
	textBlockIndex?: number | null;
}): PublicTimelinePart<T>[] {
	const events: Array<
		| { pos: number; order: number; kind: "tool"; tool: T }
		| { pos: number; order: number; kind: "progress"; step: PublicProgressStep }
		| { pos: number; order: number; kind: "text" }
	> = [];
	for (const [i, tool] of input.tools.entries())
		events.push({ pos: tool.blockIndex ?? i, order: 1, kind: "tool", tool });
	...
	if (input.text)
		events.push({ pos: input.textBlockIndex ?? Number.MAX_SAFE_INTEGER, order: 2, kind: "text" });
	events.sort((a, b) => a.pos - b.pos || a.order - b.order);
	const parts: PublicTimelinePart<T>[] = [];
	let tools: T[] = [],
		thinking = input.thinking;
	const flush = () => {
		if (tools.length || thinking) parts.push({ kind: "meta", key: tools[0]?.id || "meta", tools, thinking });
		tools = [];
		thinking = "";
	};
	for (const event of events) {
		...
		if (event.kind === "progress")
			parts.push({ kind: "progress", key: event.step.id, progress: event.step.progress });

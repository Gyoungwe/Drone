...
it.each(["file", "file-cancel", "file-elaborate"])(
	"native file submission %s respects submit vs cancel/elaborate",
	async (mode) => {
		...
		try {
			...
			await writeFile(join(cwd, "input.csv"), "id,value\na,1\n");
			...
			backend.onAskRequest((req) => {
				...
				queueMicrotask(() =>
					backend.respondAsk(
						req.id,
						req.questions[0].type === "text"
							? mode === "file-cancel"
								? { kind: "cancel" }
								: {
										kind: "answer",
										mode: mode === "file-elaborate" ? "elaborate" : "submit",
										answers: { value: { customText: join(cwd, "input.csv") } },
									}
							: {
									kind: "answer",
									mode: "submit",
									answers: { value: { values: [asks === 1 ? "同意本次请求" : "提交所需文件"] } },
								},
					),
				);
			});
			...
			faux.setResponses([
				...
				checked(
					reply([call("task_wait", { kind: "file", title: "输入数据", reason: "提供现有 input.csv" })], {
						stopReason: "toolUse",
					}),
				),
			...
			if (mode === "file") {
				...
				const latest = [...session.messages].reverse().find((m) => m.details?.taskView)?.details.taskView;
				expect(latest.tasks.find((t) => t.id === task.id).actions[0].state).toBe("pending");
			}
			expect(asks).toBe(3);

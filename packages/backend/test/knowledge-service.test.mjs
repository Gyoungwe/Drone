...
beforeEach(async()=>{
  ...
  a=join(root,'project-a');b=join(root,'project-b');vault=join(root,'Vault');app=join(root,'app-state');
  await mkdir(a);await mkdir(b);
  vi.stubEnv('PERCHO_KNOWLEDGE_DIR',app);vi.stubEnv('PI_RESEARCH_DESKTOP_CONFIG',undefined);
...
async function prepared({wiki=true}={}){
  await setup();
  if(wiki){await note('Wiki/Index.md','# Wiki\n- [[Wiki/Autotomy]]\n');await note('Wiki/Autotomy.md','# 自切\n\n已有认识。[[Library/Papers/source]]\n\n## Human review\n取样条件需要核对。\n');}
  await note('Library/Papers/source.md','# 螳蛉 自切证据\n\n切勿把物种特定结果泛化。\n');
  const service=await getKnowledgeService();const prep=await service.prepare({cwd:a,project:'project-a',query:'自切'});
...
describe('application-wide knowledge ownership',()=>{
  it('binds once in A; B inherits Vault and policies without acquiring A results path',async()=>{
    const result=await setup();const second=await loadWorkspaceConfig(b);
    expect(result.scope).toBe('application');expect(result.accessMode).toBe('application-index');
    expect(result.connectionVerified).toBe(false);expect(second.obsidianVault).toBe(result.vault);
    expect(second.resultsRoot).toBe(join(b,'results'));expect(second.knowledgeProjectId).not.toBe('project-a');
    const local=JSON.parse(await readFile(join(a,'.pi/research-workspace.json'),'utf8'));
    expect(local.knowledgeProjectId).toBe('project-a');expect(local.obsidianVault).toBeUndefined();
  ...
  it('does not adopt or silently override a legacy project Vault',async()=>{
    await mkdir(join(b,'.pi'));await writeFile(join(b,'.pi/research-workspace.json'),JSON.stringify({obsidianVault:join(root,'old'),resultsRoot:'./special'}));
    expect((await loadWorkspaceConfig(b)).obsidianVault).toBeNull();
    await setup();const cfg=await loadWorkspaceConfig(b);
    expect(cfg.obsidianVault).toBe(vault);expect(cfg.legacyProjectVault).toBe(join(root,'old'));
    expect(cfg.resultsRoot).toBe(join(b,'special'));
    await expect(saveWorkspaceConfig(b,{obsidianVault:join(root,'other')})).rejects.toThrow('application-wide');
  });
  it('preserves human Home and navigation on setup',async()=>{
    await mkdir(vault);await note('Home.md','# 我的知识库\n\n我的组织方式。\n');
    await setup();expect(await readFile(join(vault,'Home.md'),'utf8')).toContain('我的组织方式。');
  ...
  it('rejects corrupt application config instead of falling back to a project',async()=>{
    await mkdir(app);await writeFile(join(app,'binding.json'),'{bad');
  ...
  it('checks confirmation revision and rejects overlapping application data',async()=>{
    await setup();const binding=await readKnowledgeBinding();
    await expect(saveKnowledgeBinding({...binding,subagentPolicy:'none'},0)).rejects.toThrow('changed');
    await expect(configureObsidian({cwd:a,vault:join(app,'notes')})).rejects.toThrow('independent from application');
  });
  it('gives permitted subagents the application policy, not a project-only config',async()=>{
    await setup();expect(await resolveSubagentMcpAccess(b,{})).toBe('read-local');
...
describe('read-first incremental knowledge service',()=>{
  it('shares one service across projects and requires a real navigation ticket',async()=>{
    const {service}=await prepared();expect(await getKnowledgeService()).toBe(service);
  ...
  it('updates only changed content and coalesces duplicate update notifications',async()=>{
    ...
    expect((await service.search(prep.ticket,a,{query:'autotomy-beta'})).hits[0].path).toBe('Library/Papers/source.md');
  });
  it('invalidates search misses after additions and removes deleted evidence',async()=>{
    ...
    await rm(join(vault,'Library/Papers/new.md'));await service.request('changed',{paths:['Library/Papers/new.md']});
    expect((await service.search(prep.ticket,a,{query:'uniqueevidence'})).hits).toEqual([]);
  });
  it('keeps other project notes outside normal search and direct reads',async()=>{
    ...
    await expect(service.read(prep.ticket,a,{path:'Projects/other/Evidence/private.md'})).rejects.toThrow('scope');
  });
  ...
  it('does not follow child symlinks or read files outside the Vault',async()=>{
    const {service,prep}=await prepared({wiki:false});
    await writeFile(join(root,'secret.md'),'secret');await symlink(join(root,'secret.md'),join(vault,'Library/link.md'));
  ...
  it('keeps incomplete coverage visible when a note cannot be indexed',async()=>{
    ...
    expect(result.complete).toBe(false);expect(result.problems.some(x=>x.path==='Library/too-big.md')).toBe(true);
  });
  it('preserves the index on service restart without rereading unchanged bodies',async()=>{
    const {service}=await prepared({wiki:false});const before=await service.request('status');
    await closeKnowledgeServices();const next=await getKnowledgeService();const after=await next.request('reconcile');
    expect(after.noteCount).toBe(before.noteCount);expect(after.stats.bodyReads).toBe(0);
  });
  it('indexes a new deposit immediately while leaving Wiki maintenance queued',async()=>{
    const {service,prep}=await prepared({wiki:false});
    const saved=await depositKnowledge({cwd:a,project:'project-a',type:'evidence',title:'New evidence',markdown:'brandnewevidence',sourceLinks:['fixture:source']});
    ...
    expect((await service.request('jobs',{project:'project-a'})).items.some(x=>x.kind==='evidence-review')).toBe(true);
  });
  it('protects human notes and does not rewrite Wiki during navigation maintenance',async()=>{
    const {service}=await prepared();const original=await readFile(join(vault,'Wiki/Autotomy.md'),'utf8');
    await note('Library/Index.md','# Library\n\n<!-- pi-agent:managed:start -->\nold\n<!-- pi-agent:managed:end -->\n\n## Human review\n保留我的批注\n');
  ...
  it('run-only mode blocks the source archival note side path',async()=>{
    ...
    expect((await publishSourceNote({cwd:a,entry:{},runDir:root})).knowledge_status).toBe('disabled-run-only');
    await expect(depositKnowledge({cwd:a,project:'project-a',type:'paper',title:'x',markdown:'x'})).rejects.toThrow('run-only');
  });
});
...
describe('navigation delivery and bounded maintenance',()=>{
  ...
  it('automatically delivers current navigation and keeps only one context copy',async()=>{
    ...
    const messages=[{role:'user',content:'question'},{role:'custom',customType:'percho-knowledge-navigation',content:'OLD'}];
  ...
  it('removes the old navigation payload when the application binding changes',async()=>{
    await prepared();const h=harness();await h.interface.beforeStart({prompt:'x'},{cwd:a});
    await configureObsidian({cwd:b,vault:join(root,'Second Vault')});
    const output=await h.events.get('context')({messages:[]});
  ...
  it('read-only children receive no maintenance or deposition tools',async()=>{
    await setup();const h=harness(true);
  ...
  it('navigation jobs remain available under a large evidence review queue',async()=>{
    ...
    const paths=[];
    for(let i=0;i<120;i++){const path=`Library/Papers/new-${i}.md`;paths.push(path);await note(path,`# Evidence ${i}\n`);}
    for(let i=0;i<paths.length;i+=50)await service.request('changed',{paths:paths.slice(i,i+50)});
    const result=await runNavigationMaintenance(service,'project-a',10);
  ...
  it('direct application Wiki writes cannot bypass the review command',async()=>{
    await setup();
    await expect(depositKnowledge({cwd:a,project:'project-a',type:'wiki',targetScope:'shared',title:'Shared topic',markdown:'draft',sourceLinks:['fixture:source']})).rejects.toThrow('obsidian-review');
...
it('FTS-first query plan does not enumerate all notes through the scope index',async()=>{
  ...
  try {
    db.exec(schema);
    const details=db.prepare('EXPLAIN QUERY PLAN '+query).all('autotomy','project-a',5).map(row=>row.detail);

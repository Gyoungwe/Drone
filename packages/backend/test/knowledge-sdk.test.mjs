...
it('real SDK delivers global navigation before model invocation in an unrelated project',async()=>{
  ...
  const a=join(root,'A'),b=join(root,'B'),agentDir=join(root,'agent');
  let session;
  ...
  try {
    ...
    const resourceLoader=new DefaultResourceLoader({cwd:b,agentDir,settingsManager,
    ...
    await resourceLoader.reload();expect(resourceLoader.getExtensions().errors).toEqual([]);
    ...
    const noNetwork=vi.spyOn(runtime,'streamSimple').mockImplementation(()=>{throw new Error('Network not allowed');});
    ({session}=await createAgentSession({cwd:b,agentDir,settingsManager,resourceLoader,modelRuntime:runtime,
      sessionManager:SessionManager.inMemory(b),model:{id:'fixture',provider:'fixture',name:'fixture',
        api:'openai-completions',baseUrl:'https://example.invalid',input:['text'],reasoning:false,
    ...
    expect(payload).toContain('percho-knowledge-navigation');expect(payload).toContain('Wiki/Index.md');
    expect(payload).toContain(bound.vault);expect(payload).not.toContain('"ticket"');
    expect(session.getActiveToolNames()).toContain('research_search_knowledge');
    expect(errors).toEqual([]);expect(noNetwork).not.toHaveBeenCalled();
  } finally {session?.dispose();await closeKnowledgeServices();vi.restoreAllMocks();vi.unstubAllEnvs();await rm(root,{recursive:true,force:true});}
});

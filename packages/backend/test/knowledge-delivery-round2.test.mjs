...
it("correct workspace does not permit a run directory symlink outside results", async () => {
	...
	await mkdir(other);
	const link = join(cwd, "results", "topic", "run-outside");
	await symlink(other, link);
	await expect(
		tools()
			.get("research_summarize_run")
			.execute("summary", { run_dir: link, summary_markdown: "No" }, undefined, undefined, { cwd }),
...
it("sources become actual file, DOI and Vault hyperlinks without linking an unsafe path", async () => {
	...
	const result = await normalizeSourceLinks(
		...
		{ cwd, vault, resultsRoot: join(cwd, "results"), project: "project-a" },
	);
...
it("new non-Wiki deposits contain clickable existing local sources", async () => {
	...
	const result = await depositKnowledge({
		...
		project: "project-a",
		type: "software",
		title: "Fixture CLI guide",
		markdown: "## Commands\nExample only.",
		sourceLinks: ["results/topic/run-fixture/manual.txt"],
	});
	expect(await readFile(result.note, "utf8")).toContain("](<file:");
...
it("manual TOC is identified as incomplete and points to bounded same-site reference chapters", () => {
	const page =
		'<h1>Manual</h1><a href="options.html">Options</a><a href="install.html">Installation</a><a href="https://evil.invalid/commands">Commands</a><a href="javascript:alert(1)">Manual</a>';
	const value = assessManualPage(Buffer.from(page), "text/html", "https://example.invalid/manual/");
	expect(value.status).toBe("landing-or-overview-page");
...
it("even a dense reference page is never automatically declared a complete manual", () => {
	const value = assessManualPage(
		Buffer.from("<p> --input --output --threads --mode --version --help </p>"),
		"text/html",
		"https://example.invalid/options",
	);
	expect(value.status).toBe("reference-page-only");
	expect(value.complete).toBe(false);
});
it("software and paper requests have explicit deliverables and no invented benchmarks", () => {
	const mixed = deliveryContract("下载几篇DNA序列比对的方法的论文和软件说明书沉淀到知识库", {
		showMeAvailable: true,
	});
	expect(mixed.kind).toBe("mixed");
	expect(mixed.guidance).toContain("version-matched");
	expect(mixed.guidance).toContain("not tested");
	expect(mixed.guidance).toContain("show-me");
	const paper = deliveryContract("介绍一下这篇论文");
	expect(paper.kind).toBe("research");
	expect(paper.guidance).toContain("main claims");
	expect(paper.guidance).toContain("not loaded");
	expect(deliveryContract("你好")).toBeNull();
});
it("findings and artifact events show bounded facts, never raw assistant/protocol data", () => {
	const ctx = { sessionId: "round2-visible" };
	beginKnowledgeFlow(ctx, { vaultId: "v", revision: 1, vault });
	noteKnowledgeSearch(ctx, {
		query: "fixture",
		hits: [
			{
				path: "Library/Papers/source.md",
				title: "Found paper",
				text: `Actual source excerpt ${"x".repeat(2000)}`,
				hash: "a",
				startLine: 1,
				endLine: 2,
			},
	...
	noteKnowledgeOperation(ctx, {
		...
		result: {
			details: {
				...
				metadata: { title: "Downloaded paper" },
				knowledge_status: "written",
				hiddenThinking: "NO_EXPOSE",
			},
		},
	});
...
it("a Vault write failure does not erase the fact that the local summary was saved", async () => {
	...
	expect(result.details.obsidian_error).toBeTruthy();

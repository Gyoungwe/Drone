...
import { inspectObsidianSetup, inspectSetupDirectory, resolveSetupVault } from '../../../.pi/lib/obsidian-setup.mjs';
...
function harness({ input = vault, confirmed = true, hasUI = true } = {}) {
  ...
  const pi = {
    ...
    sendUserMessage: vi.fn(), sendMessage: vi.fn(),
    getCommands: vi.fn(() => [{ name: 'skill:research-vault', source: 'skill' }]),
  };
  const ctx = { cwd, hasUI, ui: { input: vi.fn(async () => input), select: vi.fn(async () => confirmed ? '确认应用此方案' : '取消，不做修改'), confirm: vi.fn(), notify: vi.fn() } };
  obsidianWorkbench(pi);
  return { pi, tools, commands, events, ctx };
}
const setupParams = () => ({ path: vault, project: 'actual-research', profile: 'hybrid', deposit_mode: 'verified', subagent_mcp: 'read-local' });
...
describe('slash command to current-model handoff', () => {
  ...
  it('discovery tool returns current project context and all three templates without writes', async () => {
    ...
    expect(result.details.binding).toMatchObject({ command: 'obsidian-setup', skill: 'research-vault', mcpServer: 'research-obsidian' });
...
describe('interactive write gate', () => {
  ...
  it('rejects headless writes', async () => {
    ...
    await expect(execute(h.tools.get('research_setup_obsidian'), setupParams(), h.ctx)).rejects.toThrow('interactive confirmation');
    await expectNoSetupWrites();
  });
  it('rejects invalid policy before asking for confirmation', async () => {
    const h = harness();
    await expect(execute(h.tools.get('research_setup_obsidian'), { ...setupParams(), subagent_mcp: 'write-all' }, h.ctx)).rejects.toThrow('Invalid');
  ...
  it('an abort before confirmation cannot initialize the Vault', async () => {
    ...
    controller.abort();
    await expect(execute(h.tools.get('research_setup_obsidian'), setupParams(), h.ctx, controller.signal)).rejects.toThrow('aborted');
    await expectNoSetupWrites();
  });
  it('an abort while awaiting confirmation cannot initialize the Vault', async () => {
    const h = harness();
    const controller = new AbortController();
    h.ctx.ui.select.mockImplementation(async () => { controller.abort(); return '确认应用此方案'; });
    const result = await execute(h.tools.get('research_setup_obsidian'), setupParams(), h.ctx, controller.signal);
    expect(result.details.cancelled).toBe(true);
    await expectNoSetupWrites();
  });
...
describe('runtime location is independent of the user project', () => {
  it('reuses an installed runtime launcher without inheriting Vault or credentials', async () => {
    ...
    await writeFile(join(installationRoot, '.mcp.json'), JSON.stringify({ mcpServers: {
      'research-obsidian': { command: 'node', args: ['server.js', '/other-vault'], env: { PRIVATE_TOKEN: 'do-not-copy' } },
    } }));
  ...
  it('fails clearly when no runtime is installed and performs no setup writes', async () => {
    await expect(resolveObsidianRuntime({ cwd, installationRoot: cwd })).rejects.toThrow('MCPVault runtime was not found');
    await expectNoSetupWrites();
  });
  it('an explicit missing runtime does not silently switch to a fallback', async () => {
    await expect(resolveObsidianRuntime({ cwd, server: './missing.js' })).rejects.toThrow('MCPVault runtime was not found');
    await expectNoSetupWrites();
  });
});
...
describe('Obsidian MCP skill binding', () => {
  ...
  it.each(['obsidian-setup', 'setup', 'research-setup'])('%s expands the same skill workflow', async name => {
    const h = harness();

...
beforeEach(async () => {
	...
	backend = new PiBackend({ projectTrust: false, permissionGates: false, desktopIntegration: {
		appendSystemPrompt: [],
...
afterEach(async () => { backend?.dispose(); vi.restoreAllMocks(); vi.unstubAllEnvs(); if (root) await rm(root, { recursive: true, force: true }); });
...
it("records the real research-workbench lazy schema footprint", async () => {
	...
	const report = { initial: initial.footprint, research: research.footprint, activeCapabilities: research.activeCapabilities };

...
const source = "Library/Papers/source-a.md",
	second = "Library/Papers/source-b.md";
const answer = (patch = {}) => ({
	summary: "Conditional finding; source limitations retained.",
	source_paths: [source],
	cautions: ["Not independently scientifically verified."],
	...patch,
});
...
function runtime(responses) {
	...
	const completeSimple = vi.fn(async (_model, ctx, _opts) => {
		...
		if (!step) throw new Error("Fixture responses exhausted");
		return typeof step === "function" ? step(ctx, _opts) : step;
	});
	return {
		contexts,
		instance: { completeSimple, getModel: () => undefined, getModels: () => [] },
		completeSimple,
	};
}
function request(patch = {}) {
	return {
		role: "evidence",
		task: "Compare assigned evidence only",
		packet: '{"sources":[]}',
		parentModel: model,
		capabilities: [],
		check: vi.fn(async () => {}),
...
describe("isolated capability runner", () => {
	it("returns only a schema-checked handoff, never private reasoning or full tool history", async () => {
		...
		const r = runtime([
			reply([hidden, call("knowledge_read", { path: source })], { stopReason: "toolUse" }),
			submit(),
		]);
		...
		expect(result.data.summary).toContain("Conditional");
	...
	it("forbids extra registered capabilities before contacting a model", async () => {
		const r = runtime([submit()]);
		await expect(
			runKnowledgeSpecialist(
				deps(r),
				request({
					capabilities: [
						{ name: "bash", description: "escape", parameters: { type: "object" }, execute: vi.fn() },
					],
				}),
			),
	...
	it.each(["bash", "read", "webfetch", "subagent", "knowledge_approve", "research_setup_obsidian"])(
		"refuses an attempted %s call",
		async (name) => {
			const r = runtime([use(name, { command: "UNAUTHORIZED", path: "/private" })]);
			await expect(runKnowledgeSpecialist(deps(r), request())).rejects.toThrow("forbidden");
			expect(r.completeSimple).toHaveBeenCalledOnce();
		},
	);
	it("refuses unstructured prose instead of pretending completion", async () => {
		const r = runtime([reply("I completed everything.")]);
		await expect(runKnowledgeSpecialist(deps(r), request())).rejects.toThrow("structured");
	});
	it("rejects oversized or authority-bearing result fields", async () => {
		const r = runtime([submit(answer({ approved: true, summary: "x".repeat(1700) }))]);
		await expect(runKnowledgeSpecialist(deps(r), request())).rejects.toThrow();
	});
	it("cannot change providers when a configured specialist model is missing", async () => {
		const r = runtime([submit()]);
		await expect(
			runKnowledgeSpecialist({ ...deps(r), getModelPreference: async () => "unavailable/model" }, request()),
		).rejects.toThrow("no provider fallback");
		expect(r.completeSimple).not.toHaveBeenCalled();
	});
	it("inherits the current model when no per-role override is configured", async () => {
		const r = runtime([submit()]);
		expect((await runKnowledgeSpecialist(deps(r), request())).model).toBe("fixture/small");
	});
	it("cancels a hung provider and does not retry", async () => {
		const r = runtime([() => new Promise(() => {})]);
		await expect(runKnowledgeSpecialist(deps(r), request({ timeoutMs: 50 }))).rejects.toThrow("timed out");
		expect(r.completeSimple).toHaveBeenCalledOnce();
		expect(r.completeSimple.mock.calls[0][2].signal.aborted).toBe(true);
	});
	it("already aborted parent work never contacts a model", async () => {
		const c = new AbortController();
		c.abort();
		const r = runtime([submit()]);
		await expect(runKnowledgeSpecialist(deps(r), request({ signal: c.signal }))).rejects.toThrow();
		expect(r.completeSimple).not.toHaveBeenCalled();
	});
	...
	it("rejects oversized task packets before provider access", async () => {
		const r = runtime([submit()]);
		await expect(runKnowledgeSpecialist(deps(r), request({ packet: "x".repeat(36001) }))).rejects.toThrow(
			"context budget",
		);
		expect(r.completeSimple).not.toHaveBeenCalled();
	});
...
beforeEach(async () => {
	root = await realpath(await mkdtemp(join(tmpdir(), "percho-specialists-")));
	...
	await note(source, "# Shared evidence\nLimited observations from study A.");
	await note(second, "# Shared evidence\nDifferent design in study B.");
	service = await getKnowledgeService();
	const prep = await service.prepare({ cwd, project: "project-a" });
	...
	ctx = { cwd, sessionId: "specialist-fixture", model, isProjectTrusted: () => true };
	beginKnowledgeFlow(ctx, current.binding);
	hostCalls = [];
	coordinator = createKnowledgeSpecialists({ getCommands: () => [] }, { getCurrent: () => current });
	coordinator.begin("比较论文中的证据");
});
afterEach(async () => {
	unregister?.();
...
function scriptHost() {
	unregister = registerKnowledgeSpecialistHost(ctx.sessionId, async (req) => {
		...
		const responses =
			req.role === "navigator"
				? [
						use("knowledge_read", { path: "Wiki/Topic.md" }),
						use("knowledge_search", { query: "evidence" }),
						reply([call("knowledge_read", { path: source }), call("knowledge_read", { path: second })], {
							stopReason: "toolUse",
						}),
						submit(answer({ source_paths: [source, second] })),
					]
				: [submit(answer({ source_paths: [source, second] }))];
...
describe("host-owned knowledge delegation", () => {
	it("automatically orients and compares once, but never grants parent evidence receipts", async () => {
		scriptHost();
		const first = await coordinator.orient(ctx),
			again = await coordinator.orient(ctx);
		expect(hostCalls.map((c) => c.role)).toEqual(["navigator", "evidence"]);
	...
	it("respects the Vault none ceiling even for a manually requested specialist", async () => {
		scriptHost();
		current.binding = { ...current.binding, subagentPolicy: "none" };
		expect((await coordinator.run(ctx, "navigator")).reason).toBe("permission-denied");
		expect(hostCalls).toHaveLength(0);
	});
	it("untrusted projects cannot delegate Vault content", async () => {
		scriptHost();
		ctx.isProjectTrusted = () => false;
		expect((await coordinator.run(ctx, "navigator")).status).toBe("skipped");
		expect(hostCalls).toHaveLength(0);
	});
	it("human manual/off settings are enforced and use optimistic revisions", async () => {
		scriptHost();
		const initial = await specialistSettings();
		const manual = await setSpecialistSettings({
			mode: "manual",
	...
	it("does not add a recursive Show Me request when the skill is unavailable", async () => {
		scriptHost();
		expect((await coordinator.run(ctx, "explainer", { sourcePaths: [source] })).reason).toBe(
			"show-me-not-loaded",
		);
		expect(hostCalls).toHaveLength(0);
	});
	it("a forged result path cannot become an accepted source handoff", async () => {
		unregister = registerKnowledgeSpecialistHost(ctx.sessionId, async () => ({
			data: answer({ source_paths: ["Library/Papers/invented.md"] }),
			usage: {},
		}));
		const result = await coordinator.run(ctx, "evidence", { sourcePaths: [source] });
		expect(result.status).toBe("failed");
		expect(result.reason).toContain("unread");
	});
	it("source changes during a specialist run invalidate its completion", async () => {
		unregister = registerKnowledgeSpecialistHost(ctx.sessionId, async () => {
			await note(source, "# Changed source");
	...
	it("derived explainers and run summaries cannot enter an evidence packet", async () => {
		scriptHost();
		const a = await coordinator.run(ctx, "evidence", { sourcePaths: ["Library/Explainers/topic.md"] });
		expect(a.status).toBe("failed");
		coordinator.begin("new turn");
		const b = await coordinator.run(ctx, "evidence", { sourcePaths: ["Projects/project-a/Runs/x.md"] });
	...
	it("cancels in-flight work when a new user turn replaces the parent context", async () => {
		...
		const work = coordinator.run(ctx, "evidence", { sourcePaths: [source] });
		await ready;
		coordinator.begin("another topic");
		expect((await work).status).toBe("cancelled");
	});
	it("one role cannot be repeatedly invoked in the same user turn", async () => {
		scriptHost();
		await coordinator.run(ctx, "navigator");
		expect((await coordinator.run(ctx, "navigator")).reason).toBe("already-called-this-turn");
		expect(hostCalls).toHaveLength(1);
	});
	it("does not dispatch specialists for greetings or control commands", () => {
		for (const text of ["你好", "同意", "/obsidian-setup", "取消任务"])
			expect(shouldOrientKnowledge(text)).toBe(false);
		for (const text of ["介绍这篇论文", "Compare these papers", "根据知识库解释这个方法", "下载软件说明书"])
			expect(shouldOrientKnowledge(text)).toBe(true);
	});
	it("protected built-in identities cannot be widened by user or project Markdown", async () => {
		...
		await writeFile(
			join(agentDir, "agents/knowledge-navigator.md"),
		...
		const nav = agents.find((a) => a.name === "knowledge-navigator");
		expect(nav.source).toBe("builtin");
		expect(nav.tools).not.toContain("bash");
...
describe("bounded scheduling and generated artifacts", () => {
	it("shares two slots across sessions and removes a cancelled queued task", async () => {
		...
		const first = withSpecialistSlot(undefined, work),
			secondRun = withSpecialistSlot(undefined, work);
		const abort = new AbortController(),
			third = withSpecialistSlot(abort.signal, work);
		abort.abort();
...
describe("shared native child ceiling", () => {
	it("does not exceed the configured project ceiling even across different specialist lanes", async () => {
		...
		await writeFile(
			...
			JSON.stringify({ ...config, maxConcurrentSubagents: 1 }),
		);
		let active = 0,
			max = 0;
		const work = async () => {
	...
	it("all native agent work shares three slots across workspaces", async () => {
		...
		await Promise.all(
			Array.from({ length: 7 }, (_, i) =>
				withNativeSubagentSlot(join(root, `workspace-${i}`), undefined, work),
			),
		);
		expect(max).toBe(3);
	});
...
it("long parent research does not consume the later specialist delivery budget", async () => {
	scriptHost();
	const now = vi.spyOn(Date, "now").mockReturnValue(Date.now() + 10 * 60 * 1000);
	try {
		expect((await coordinator.run(ctx, "evidence", { sourcePaths: [source, second] })).status).toBe(
			"completed",
		);
	} finally {
		now.mockRestore();
		...
		sources: [
			{
				...
				hiddenThinking: "HIDDEN_SOURCE_FIELD",
			},
		],
	});
	const row = flowFor(ctx.sessionId).specialists[0];
	expect(row.sources[0].excerpt.length).toBe(280);
	expect(row.sources[0].path).toBe(source);
...
describe("overlapping requests and replacement turns", () => {
	it("reserves an explainer role before asynchronous skill loading so concurrent delegates cannot fan out twice", async () => {
		...
		await mkdir(join(root, "show-me"));
		await writeFile(skill, "---\nname: show-me\n---\nUse static HTML.");
		coordinator.close();
		coordinator = createKnowledgeSpecialists(
			{ getCommands: () => [{ source: "skill", name: "skill:show-me", sourceInfo: { path: skill } }] },
			{ getCurrent: () => current },
		);
		...
		const results = await Promise.all([
			coordinator.run(ctx, "explainer", { sourcePaths: [source] }),
			coordinator.run(ctx, "explainer", { sourcePaths: [source] }),
		]);
		expect(calls).toBe(1);
	...
	it("cancelled orientation cannot reinsert the previous task report into the new user turn", async () => {
		...
		unregister = registerKnowledgeSpecialistHost(ctx.sessionId, async (req) => {
			announce();
			return new Promise((_, reject) =>
				req.signal.addEventListener("abort", () => reject(new Error("old task cancelled")), { once: true }),
			);
		});
		const original = coordinator.orient(ctx);
		await started;
		coordinator.begin("你好");
		expect(await original).toEqual([]);
		expect(await coordinator.orient(ctx)).toEqual([]);
	});
	it("work suspended on initial settings IO is discarded rather than consuming the replacement turn", async () => {
		scriptHost();
		const original = coordinator.run(ctx, "evidence", { sourcePaths: [source, second] });
		coordinator.begin("Replacement turn");
...
it("uses and identifies the bundled research-show-me fallback without a user skill installation", async () => {
	const skill = join(root, "research-show-me/SKILL.md");
	await mkdir(join(root, "research-show-me"));
	await writeFile(skill, "---\nname: research-show-me\n---\nFirst-party portable presentation fallback.");
	coordinator.close();
	coordinator = createKnowledgeSpecialists(
		{ getCommands: () => [{ source: "skill", name: "skill:research-show-me", sourceInfo: { path: skill } }] },
		{ getCurrent: () => current },
	);
	coordinator.begin("Explain source");
	let sent;
	unregister = registerKnowledgeSpecialistHost(ctx.sessionId, async (request) => {
		sent = request;
		return { data: answer(), usage: {} };
	});
	const result = await coordinator.run(ctx, "explainer", { sourcePaths: [source] });

...
it('Obsidian command expands its skill, bridges questions and gates fixture Vault writes', async () => {
  ...
  const cwd = join(root, 'actual-project'), vault = join(root, 'Knowledge Vault');
  const agentDir = join(root, 'agent');
  const requests = [], errors = [];
  const askGate = new AskGate(request => { requests.push(request); return true; });
  let session;
  ...
  try {
    ...
    const noNetwork = vi.spyOn(runtime, 'streamSimple').mockImplementation(() => { throw new Error('Unexpected provider request'); });
    ({ session } = await createAgentSession({ cwd, agentDir, resourceLoader, settingsManager,
      sessionManager: SessionManager.inMemory(cwd), modelRuntime: runtime,
      model: { id: 'fixture', provider: 'fixture', name: 'fixture', api: 'openai-completions',
    ...
    const questions = Object.entries(choices).map(([id, value]) => ({ id, prompt: `Confirm ${id}`,
      type: 'single', required: true, options: [{ value, label: value }] }));
    const interview = tool('ask_user').execute('interview', { title: 'Obsidian MCP · 项目知识结构', questions });
    await vi.waitFor(() => expect(requests).toHaveLength(2));
    expect(requests[1].questions).toHaveLength(3);
    askGate.respond(requests[1].id, { kind: 'answer', answers: Object.fromEntries(
      Object.entries(choices).map(([id, value]) => [id, { values: [value] }])) });
    ...
    const applying = tool('research_setup_obsidian').execute('apply', {
      path: vault, project: 'fixture-research', ...choices });
    ...
    askGate.dispose(); session?.dispose(); vi.restoreAllMocks(); vi.unstubAllEnvs();

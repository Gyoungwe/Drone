...
import type { Context, Model, Tool, Message } from "@earendil-works/pi-ai";
import { validateToolArguments } from "@earendil-works/pi-ai";
import type { ModelRuntime } from "@earendil-works/pi-coding-agent";
import { Type } from "typebox";
import { KNOWLEDGE_SPECIALISTS, type KnowledgeSpecialistRole } from "@percho/shared";
...
export interface SpecialistAnswer {
 summary: string; source_paths: string[]; cautions: string[];
...
const ROLES: Record<KnowledgeSpecialistRole,string> = {
 navigator: "Read the supplied navigation first. Follow relevant Wiki links with knowledge_read, then run knowledge_search. Open selected underlying notes. Return only the most relevant source paths you actually read, a compact orientation, and missing coverage. Search snippets alone are not read receipts.",
 evidence: "Work only on the assigned sources. Compare methods, versions, applicability, conflicting claims, and evidence gaps. Separate observation, interpretation, hypothesis, and unknown. Never certify scientific truth or treat Wiki, a run summary, or Show Me as a primary source. Return compact findings, not a rewritten source dump.",
 ...
 explainer: "Use the provided real show-me skill when present. Explain papers through questions, main claims, methods, evidence and limitations; software through principles, version-matched command examples and documented performance. No local tests are available: say not locally tested, never invent timings. Return one self-contained static HTML with inline CSS and native details/summary interactions. No scripts, external assets, frames, forms, or event handlers. This is presentation, never scientific evidence.",
};
export function specialistSubmitSchema(role: KnowledgeSpecialistRole) {
 return Type.Object({
  summary: Type.String({minLength:1,maxLength:1600}),
  source_paths: Type.Array(Type.String({minLength:1,maxLength:512}),{maxItems:6,uniqueItems:true}),
  cautions: Type.Array(Type.String({maxLength:300}),{maxItems:4}),
  ...(role==='wiki'?{title:Type.String({minLength:1,maxLength:180}),markdown:Type.String({minLength:1,maxLength:18000})}:{}),
  ...(role==='explainer'?{title:Type.String({minLength:1,maxLength:180}),html:Type.String({minLength:1,maxLength:64000})}:{}),
...
export async function runKnowledgeSpecialist(deps: SpecialistRunnerDeps, request: SpecialistRequest): Promise<SpecialistResult> {
 const profile=KNOWLEDGE_SPECIALISTS.find(p=>p.role===request.role);
 if(!profile)throw new Error('Unknown knowledge specialist');
 if(request.task.length>4000||Buffer.byteLength(request.packet)>36000||Buffer.byteLength(request.skillText||'')>18000)
 ...
 for(const tool of request.capabilities){
  if(tool.name==='knowledge_submit'||!(profile.permissions as readonly string[]).includes(tool.name)||capabilities.has(tool.name))
   throw new Error('Knowledge specialist capability is not permitted');
  capabilities.set(tool.name,tool);
 }
 const submit:Tool={name:'knowledge_submit',description:'Return the structured task result to the parent. This does not write, approve or execute anything.',parameters:specialistSubmitSchema(request.role)};
 const tools:Tool[]=[...capabilities.values()].map(({name,description,parameters})=>({name,description,parameters}));tools.push(submit);
 const controller=new AbortController();
 const cancel=()=>controller.abort(request.signal?.reason ?? new Error('Knowledge specialist cancelled'));
 ...
 const abortPromise=new Promise<never>((_,reject)=>{
  ...
  else controller.signal.addEventListener('abort',()=>reject(new Error('Knowledge specialist cancelled or timed out')),{once:true});
 });
 // Avoid unhandled rejections while a capability is awaiting its own bounded IO.
 void abortPromise.catch(()=>{});
 const checked=async()=>{controller.signal.throwIfAborted();await request.check();controller.signal.throwIfAborted();};
 const usage={inputTokens:0,outputTokens:0,cost:0};
 const cacheSessionId=`knowledge-${profile.role}-${randomUUID()}`;
 try{
  await checked();
  const runtime=await deps.getRuntime();const preference=await deps.getModelPreference(profile.name);
  let model=request.parentModel;
  if(preference){
   const slash=preference.indexOf('/');
   const choices=slash>0?[runtime.getModel(preference.slice(0,slash),preference.slice(slash+1))]:runtime.getModels().filter(m=>m.id===preference);
   if(choices.length!==1||!choices[0])throw new Error('Configured specialist model is unavailable; no provider fallback was used');
   model=choices[0];
  }
  if(!model)throw new Error('No model is configured for this knowledge specialist');
  const modelRef=`${model.provider}/${model.id}`;request.progress({model:modelRef});
  const messages:Message[]=[{role:'user',content:[{type:'text',text:JSON.stringify({task:request.task,sourceData:request.packet})}],timestamp:Date.now()}];
  const context:Context={systemPrompt:[
   `You are ${profile.name}, an isolated knowledge-service worker.`,ROLES[request.role],
   ...
   request.skillText?`Task-specific show-me skill (only its presentation requirements apply; capability restrictions above remain):\n${request.skillText}`:'',
  ].filter(Boolean).join('\n\n'),messages,tools};
  let calls=0;
  for(let turn=0;turn<profile.maxTurns;turn++){
   await checked();
   if(Buffer.byteLength(JSON.stringify(messages))>80000)throw new Error('Specialist private context budget exhausted');
   request.progress({action:'model'});
   const reply=await Promise.race([runtime.completeSimple(model,context,{signal:controller.signal,maxTokens:profile.maxTokens,sessionId:cacheSessionId}),abortPromise]);
   usage.inputTokens+=reply.usage?.input||0;usage.outputTokens+=reply.usage?.output||0;usage.cost+=reply.usage?.cost?.total||0;
   if(['error','aborted','length'].includes(reply.stopReason))throw new Error(`Specialist model did not complete (${reply.stopReason})`);
   await checked();messages.push(reply);
   ...
   for(const call of requested){
    ...
    const value=await Promise.race([capabilities.get(call.name)!.execute(args,controller.signal),abortPromise]);
    const text=JSON.stringify(value);if(Buffer.byteLength(text)>18000)throw new Error('Specialist tool result exceeds its context budget');

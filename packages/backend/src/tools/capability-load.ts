...
export interface CapabilityLoadDetails extends CapabilityState { uiOnly: false }
...
export function makeCapabilityLoadTool(runtime: CapabilityRuntime): ToolDefinition<typeof params> {
	...
	return {
		...
		promptSnippet: "capability_load({capabilities:[knowledge|research|coding|web|files|visualization|external]})",
		parameters: params,
		execute: async (_id, input): Promise<AgentToolResult<CapabilityLoadDetails>> => {
			...
			const skills = state.visibleSkills.length ? `; skills: ${state.visibleSkills.slice(0, 12).join(", ")}` : "";
			return {
				content: [{ type: "text", text: `Capabilities active: ${state.activeCapabilities.join(", ") || "core"}; ${state.activeTools.length} tools available${skills}.` }],

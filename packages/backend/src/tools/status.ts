...
const statusParams = Type.Object({
 kind:Type.Optional(Type.Union([Type.Literal("plan"),Type.Literal("update"),Type.Literal("summary")],{description:"Public stage plan, progress update, or short completed-stage summary; not internal reasoning."})),
 detail:Type.Optional(Type.String({maxLength:400,description:"Brief public plan/progress explanation: goal, evidence gap, or observable reason for next action. Never hidden chain-of-thought, internal deliberation or unsupported conclusions."})),
 next:Type.Optional(Type.String({maxLength:160,description:"Next visible action, not a claim of completed work."})),
	text: Type.String({
		minLength: 1,
		maxLength: 60,
...
export interface StatusToolDetails {
	...
	phase: string;
	uiOnly: true;
 detail?:string;next?:string;kind?:"plan"|"update"|"summary";
}
	...
	return {
		...
		execute: async (_id, params): Promise<AgentToolResult<StatusToolDetails>> => {
			...
			return {
				...
				details: { status, phase: params.phase ?? "other", uiOnly: true,...(params.kind?{kind:params.kind}:{}),
 ...(params.detail?{detail:params.detail.trim()}:{}),...(params.next?{next:params.next.trim()}:{}) },
			};
		},
	};
}

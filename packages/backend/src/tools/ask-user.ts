...
const optionSchema = Type.Object({
	value: Type.String(), label: Type.String(), description: Type.Optional(Type.String()),
	preview: Type.Optional(Type.String()), recommended: Type.Optional(Type.Boolean()),
});
const questionSchema = Type.Object({
	id: Type.String(), label: Type.Optional(Type.String()), prompt: Type.String(),
	type: Type.Optional(Type.Union([Type.Literal("single"), Type.Literal("multi"), Type.Literal("preview")])),
	required: Type.Optional(Type.Boolean()), options: Type.Array(optionSchema, { minItems: 1 }),
});
const paramsSchema = Type.Object({ title: Type.Optional(Type.String()), questions: Type.Array(questionSchema, { minItems: 1 }) });

type RawParams = { title?: string; questions: Array<{ id: string; label?: string; prompt: string; type?: AskQuestion["type"]; required?: boolean; options: AskQuestion["options"] }> };
...
function normalizeQuestions(params: RawParams): AskQuestion[] {
	...
	return params.questions.map((q, index) => {
		...
		const options = q.options.map((option) => {
			...
			if (!value || values.has(value)) throw new Error(`Question ${index + 1}: option values must be unique and non-empty`);
			values.add(value);
			return { ...option, value, label: option.label.trim() || value, description: option.description?.trim() || undefined, preview: option.preview?.trim() || undefined };
		});
		const type = q.type ?? "single";
		if (type === "preview" && options.some((option) => !option.preview)) throw new Error(`Question ${index + 1}: preview questions require preview text for every option`);
		return { id, label: q.label?.trim() || `Q${index + 1}`, prompt: q.prompt.trim(), type, required: q.required ?? false, options };
	});
}
...
function resultFromResponse(params: RawParams, questions: AskQuestion[], response: AskResponse) {
	...
	for (const [id, input] of Object.entries(response.answers)) {
		...
		const selected = (input.values ?? []).map((value) => {
			...
			return { value, label: question.options[index]!.label, index: index + 1 };
		});
		const customText = input.customText?.trim() ? input.customText : undefined;
		if (question.type === "single" && selected.length > 1) throw new Error(`Question "${id}" accepts one option`);
		if (!selected.length && !customText && !input.note?.trim()) continue;
		const selectedNotes = input.optionNotes ? Object.fromEntries(selected.map((item) => [item.value, input.optionNotes?.[item.value]]).filter((entry): entry is [string, string] => Boolean(entry[1]))) : undefined;
		answers[id] = {
			...
			...(customText ? { customText } : {}), ...(input.note?.trim() ? { note: input.note } : {}),
			...(selectedNotes && Object.keys(selectedNotes).length ? { optionNotes: selectedNotes } : {}),
		};
	}
	return { title: params.title, cancelled: false, mode: response.mode ?? "submit", questions: questions.map(summary), answers };
}

function summary(q: AskQuestion) { return { id: q.id, label: q.label, prompt: q.prompt, type: q.type }; }
...
export function makeAskUserTool(deps: AskUserToolDeps): ToolDefinition<typeof paramsSchema> {
	return {
		...
		description: "Ask the user structured clarification questions in the Percho desktop UI before proceeding.",
		promptSnippet: "Clarify ambiguous or preference-sensitive decisions with a short interactive interview before proceeding",
		parameters: paramsSchema,
		async execute(toolCallId, params, signal): Promise<AgentToolResult<unknown>> {
			const raw = params as RawParams;
			const questions = normalizeQuestions(raw);
			const response = await deps.ask({ toolCallId, title: raw.title?.trim() || undefined, questions }, signal);

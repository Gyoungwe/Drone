...
export function McpPanel() {
	...
	const serverMap = useMemo(
		() => new Map((status?.servers ?? []).map((server) => [server.name, server])),
		[status],
	);
		...
		<div>
			...
			<div className="mt-5">
				<div className="flex items-center justify-between gap-3">
					...
					<button
						type="button"
						className="flex shrink-0 items-center gap-1.5 rounded-lg border border-border px-2.5 py-1.5 text-[11px] text-ink-2 transition-colors hover:border-border-strong hover:bg-hover"
						onClick={() => void window.pi.openMcpConfig(cwd ?? undefined)}
					>
										...
										<div className="mt-1 flex min-w-0 items-center gap-1.5 text-[10px] text-ink-faint">
											<span
												className={
													server.scope === "project"
														? "rounded bg-accent/10 px-1.5 py-0.5 font-medium text-accent"
														: "rounded bg-hover px-1.5 py-0.5 text-ink-2"
												}
											>
												{t(`settings.mcp.scope.${server.scope}`)}
											</span>
											...
											{runtime ? (
												<>
													<span>·</span>
													<span>
														{runtime.toolCount} {t("settings.mcp.tools")}
													</span>
												</>
											) : null}
										</div>

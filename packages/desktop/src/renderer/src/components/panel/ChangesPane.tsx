...
import { ResourceSidebar } from "../diff/ResourceSidebar";
...
/**
 * 「变更」页签（原 DiffSidebar 内容）：分支行 + 插件区 + 分段开关「全部 / 最近一轮」+ 按轮倒序的文件卡。
 * chip 跳转 = stores/ui.ts 的 diffFocus → 这里直接操作 DOM（开卡片 + 滚动定位 + 闪烁）。
 * 有资源预览（resourcePreview）时整页切成 ResourceSidebar（文件/报告预览），返回即回到 diff。
 */
export function ChangesPane({ sessionId }: { sessionId: string | null }) {
	const t = useT();
	...
	const messages = useTranscriptStore((s) => (sessionId ? s.bySession[sessionId]?.messages : undefined));
	...
	// chip → 跳转：目标不在当前分段时先切「全部」（effect 随 visibleTurns 变化重跑），再定位
	useEffect(() => {
		if (!diffFocus) return;
	...
	if (resourcePreview) {
		return (
			<div className="context-pane-fill resource-sidebar">
	...
	return (
		<div className="context-pane-fill">
			<div className="diff-side-head">
				...
				<div className="diff-seg" role="tablist">
					...
					<button type="button" className={scope === "latest" ? "on" : ""} onClick={() => setScope("latest")}>
						{t("diff.scopeLatest")}
					</button>
				</div>
			</div>

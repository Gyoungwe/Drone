...
const phases: Record<string,keyof typeof knowledgeZh> = {
 'setup-complete':'setupComplete',preparing:'preparing',navigation:'navigationPhase','reading-wiki':'readingWiki',searching:'searching','reading-evidence':'readingEvidence',
...
export function KnowledgeFlowCard({sessionId}:{sessionId:string|null}){
 ...
 const activeSpecialist=flow.specialists?.find(agent=>agent.status==='running'||agent.status==='queued');
 const title=activeSpecialist?`${t(`specialist_${activeSpecialist.role}`)} · ${t(`worker_${activeSpecialist.status}`)}`:t(phases[flow.phase]||'flow');
 ...
 return <section className="mx-4 mb-2 mt-2 shrink-0 rounded-xl border border-border bg-surface text-ink" data-testid="knowledge-flow-card" aria-label={t('flow')}>
  ...
  {open&&<div className="max-h-[36vh] overflow-auto border-t border-border px-3 py-3">
   ...
   {!!flow.specialists?.length&&<div className="mb-3" data-testid="knowledge-specialist-runs"><h4 className="mb-2 text-[11px] font-semibold">{t('specialistRuns')}</h4>{flow.specialists.map(agent=><div key={agent.id} className="mb-2 rounded-lg border border-border p-2 text-[11px]"><div className="flex justify-between gap-2"><strong>{t(`specialist_${agent.role}`)}</strong><span>{t(`worker_${agent.status}`)}</span></div><p className="mt-1 break-all text-[10px] text-ink-faint">{agent.model||agent.name}{agent.action?' · '+agent.action:''}</p>{agent.summary&&<p className="mt-1 break-words">{agent.summary}</p>}{agent.error&&<p className="mt-1 break-words text-warn">{agent.error}</p>}{agent.sources?.map(ref=><div key={ref.path} className="mt-2 border-t border-border pt-1"><div className="flex items-center justify-between gap-2"><span className="min-w-0 truncate font-mono text-[10px]" title={ref.path}>{ref.path} · L{ref.startLine}–{ref.endLine}</span><Button size="sm" onClick={()=>setPath(ref.path)}>{t('openSource')}</Button></div>{ref.excerpt&&<p className="break-words text-[10px] text-ink-dim">{ref.excerpt}</p>}</div>)}{agent.inputTokens!==undefined&&<p className="mt-1 text-[10px] text-ink-dim">{t('specialistUsage')} {agent.inputTokens} / {agent.outputTokens??0} · {t('sources')} {agent.sourceCount??0}</p>}</div>)}<p className="text-[10px] text-ink-faint">{t('specialistUnverified')}</p></div>}
   <h4 className="mb-2 text-[11px] font-semibold">{t('findings')}</h4>
   {(flow.search?.previews||[]).map(row=><div key={row.path} className="mb-2 rounded-lg border border-border p-2 text-[11px]"><div className="flex items-start justify-between gap-2"><strong className="min-w-0 break-words">{row.title||row.path}</strong><Button size="sm" onClick={()=>setPath(row.path)}>{t('openSource')}</Button></div><p className="whitespace-pre-wrap break-words text-ink-dim">{row.excerpt}</p><p className="mt-1 break-all font-mono text-[10px] text-ink-faint">{row.path} · L{row.startLine}–{row.endLine}</p></div>)}
   ...
   {!!flow.artifacts?.length&&<div className="mb-3"><h4 className="mb-1 text-[11px] font-semibold">{t('outputs')}</h4>{flow.artifacts.map(item=><div key={item.key} className="mb-1 rounded-lg bg-hover p-2 text-[11px]"><div className="flex items-start justify-between gap-2"><span className="min-w-0 break-words">{item.title} · {item.status}</span>{item.path&&<Button size="sm" onClick={()=>{if(/^(Wiki|Library|Projects|Inbox|Indexes)\//.test(item.path||''))setPath(item.path);else useUiStore.getState().openResourcePreview({href:item.path||'',label:item.title,cwd:cwd||undefined});}}>{t('openSource')}</Button>}</div><p className="break-words text-ink-dim">{item.detail}</p></div>)}</div>}

...
export function KnowledgePanel({context}:{context?:{cwd:string|null;sessionId:string|null;tab?:'overview'|'reviews'|'maintenance';id?:string}}){
 ...
 async function inspect(){
  setBusy(true);setActionError(null);try{setPreview(await getPi().previewKnowledgeSetup({cwd,path:path.trim()||binding?.vault||null}));}catch(e){setActionError(String((e as Error).message||e));}finally{setBusy(false);}
 }
 ...
 return <div className="text-ink" data-testid="knowledge-panel">
  ...
  {tab==='overview'&&<div className="space-y-4">
   ...
   {binding&&<KnowledgeSpecialists settings={data.specialistSettings} bindingRevision={binding.revision} permitted={binding.subagentPolicy==='read-local'} refresh={refresh}/>}
   {data?.specialistSettingsError&&<p role="alert" className="text-xs text-err">{data.specialistSettingsError}</p>}
   ...
   <section className="rounded-xl border border-border p-4"><h3 className="text-xs font-semibold">{t('newPath')}</h3><p className="mt-1 text-[10px] text-ink-dim">{t('skillBound')}</p><p className="my-2 text-[11px] leading-relaxed text-ink-dim">{t('globalSwitch')}</p>
    ...
    <div className="mt-2 flex flex-wrap gap-1"><Button size="sm" disabled={busy} onClick={()=>void folders()}>{t('selectFolder')}</Button><Button size="sm" disabled={busy} onClick={()=>void inspect()}>{busy?t('loading'):t('preview')}</Button></div>
    ...
    {preview&&<div className="mt-3 space-y-3"><p className="text-[11px] text-ink-dim">{t('previewOnly')}</p><div><p className="text-xs font-medium">{preview.context.vault?.exists?t('existing'):t('newVault')}</p><pre className="mt-1 max-h-44 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-hover p-3 text-[11px]">{(preview.context.vault?.entries||[]).map(e=>(e.type==='directory'?'▸ ':'  ')+e.path).join('\n')||t('emptyPreview')}</pre>{preview.context.vault?.truncated&&<p className="text-[10px] text-warn">{t('truncated')}</p>}</div>
     <div><h4 className="mb-1 text-xs font-semibold">{t('templates')}</h4>{preview.options.profiles.map(profile=><details key={profile.id} open={profile.id===(binding?.profile||'hybrid')} className="mt-1 rounded-lg border border-border p-2 text-xs"><summary className="cursor-pointer">{profile.label}</summary><p className="mt-2 text-ink-dim">{profile.description}</p><p className="mt-2 text-[11px]">{t('library')}：{profile.libraryTypes.join(' · ')}</p><p className="mt-1 text-[11px]">{t('projectFolders')}：{profile.projectTypes.join(' · ')}</p><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap rounded-lg bg-hover p-3 font-mono text-[11px]">{profile.directories.join('\n')}</pre></details>)}</div>
     <div><h4 className="text-xs font-semibold">{t('sampleContent')}</h4>{preview.templateSamples.map(sample=><details key={sample.path} className="mt-1 rounded-lg border border-border p-2"><summary className="cursor-pointer font-mono text-xs">{sample.path}</summary><pre className="mt-2 max-h-52 overflow-auto whitespace-pre-wrap break-words bg-hover p-3 text-[11px]">{sample.text}</pre></details>)}</div>
    </div>}

...
export function KnowledgeUiRoot() {
	...
	useEffect(() => {
		...
		const focus = () => box.current?.querySelector<HTMLElement>("button")?.focus();
			...
			{dialog &&
				createPortal(
					...
					>
						<div
							ref={box}
							className="flex max-h-[88vh] w-full max-w-[1120px] flex-col overflow-hidden rounded-2xl border border-border bg-surface text-ink shadow-dialog"
						>
							<div className="flex shrink-0 items-center justify-between border-b border-border px-5 py-3">
								<p className="text-xs font-medium">Obsidian · {t("title")}</p>
								<Button size="sm" onClick={close} aria-label={t("close")}>
									×
								</Button>
							</div>
			...
			{notice &&
				createPortal(
					<div
						...
						className="fixed bottom-5 right-4 z-[80] w-[min(420px,calc(100vw-32px))] rounded-xl border border-border bg-surface p-4 text-ink shadow-dialog"

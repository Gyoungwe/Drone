...
const message=(text:string)=>({role:"assistant",content:[{type:"text",text}],timestamp:42,stopReason:"stop"});
describe("authoritative non-streaming final snapshots",()=>{
 ...
 it("replaces stale deltas with the checked final notice",()=>{
  let s=reduceEvent(emptyTranscript(),ev("turn_start"));
  s=reduceEvent(s,ev("message_update",{assistantMessageEvent:{type:"text_delta",delta:"old delta",contentIndex:0}}));
 ...
 it("hydrates from turn_end when message_end is unavailable",()=>{
  const s=reduceEvent(emptyTranscript(),ev("turn_end",{message:message("Final snapshot"),toolResults:[]}));
 ...
 it("does not treat custom navigation and tool results as assistant text",()=>{
  let s=reduceEvent(emptyTranscript(),ev("turn_start"));
  s=reduceEvent(s,ev("message_end",{message:{role:"toolResult",content:[{type:"text",text:"tool data"}]}}));

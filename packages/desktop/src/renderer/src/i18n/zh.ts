...
export const zh = {
	taskConsent: {
		manual: "手动控制与诊断（需要时展开）",
		title: "一次确认本任务",
		active: "本任务已授权 · 阶段自动推进",
		limits: "本任务总上限 {calls} 次工具调用；最多 3 次自动恢复。不会自动扩大预算。",
		directories: "允许常规写入的目录（含子目录）：",
		noDirectories: "未申请额外写入目录；文件操作仍按现有权限处理。",
		boundary:
			"确认同时批准下方验收方案。命令沿用现有权限——若仍弹出命令确认，选「本次任务全部允许」即可不再逐次询问；删除、敏感文件、越界操作、新风险及实际人工审核不在自动授权内，可随时停止。",
		approve: "确认一次，执行到交付",
	},
	common: {
		save: "保存",

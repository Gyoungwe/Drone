...
export const zh = {
	...
	panel: {
		...
		subagents: {
			...
			refresh: "刷新",
			openDir: "agents/ 目录",
			noSession: "先发送一条消息创建会话，子智能体只能派发到已存在的会话。",
			readOnly: "只读会话不能派发子智能体。",
			empty: "没有可用的子智能体。在 {user} 或项目 {project} 添加 *.md 定义。",
			loadError: "无法读取子智能体列表：{error}",
			source: {
				builtin: "内置",
				user: "用户",
				project: "项目",
			...
			untrusted: "项目 · 未信任",
			untrustedHint: "当前项目未受信任：项目级子智能体（{n} 个）需要在派发时逐次勾选确认；不会写入信任文件。",
			tools: "工具",
			mcp: "MCP",
			mcpNone: "无",
			mcpReadLocal: "只读访问（研究策略）",
			model: "模型",
			modelInherit: "跟随主会话 · 在设置中覆盖",
			definition: "定义",
			builtinDefinition: "内置（不可编辑）",
			dispatchHere: "派发到本会话",
			dispatch: "派发",
			dispatchN: "派发 {n} 个任务",
			dispatchQueue: "派发（加入队列）",
			addTask: "+ 并行任务",
			removeTask: "移除任务",
			taskN: "任务 {n}",
			agent: "子智能体",
			task: "任务",
			taskPlaceholder: "要交给它做的事：目标、范围、产出位置…",
			stage: "阶段提示",
			stageNone: "不追加阶段契约",
			stageHint: "—— 阶段契约会追加在末尾：{contract}",
			cwd: "工作目录",
			cwdCurrent: "当前会话",
			requiredTools: "必需工具",
			followUp: "完成后把摘要交给主模型继续（followUp）",
			trustProject:
				"我信任本项目的 .pi/agents/ 定义，允许在此会话运行项目级子智能体（仅本次派发，不写入信任文件）",
			insertToComposer: "插入到输入框",
			insertHint: "保底路径：把结构化请求写进输入框，由主模型决定是否调用 subagent 工具",
			slots: "运行槽",
			slotsQueued: "本批 {now} 个立即运行，{queued} 个排队",
			slotsFull: "运行槽 {limit}/{limit} 已占用，派发后排队",
			footnote:
				"派发不需要主模型空闲；结果按 followUp 队列进入上下文。子智能体的审批仍会出现在输入框位置的审批坞。",
			errors: {
				agent: "请选择子智能体",
				task: "请填写任务",
				tools: "{agent} 的工具集里没有 {tools}，请移除或换一个子智能体",
				trust: "项目级子智能体需要勾选信任后才能派发",
				untrusted: "{agent} 是未信任项目的项目级定义，不能派发",
			},
			fullAccessHint: "本会话处于完全访问模式：子智能体同样不再逐项确认，其高危操作会写入审计日志。",
			dispatchFailed: "派发被拒绝：{error}",
			openPermissions: "查看权限设置",
			filterActive: "运行中",
			filterAll: "全部",
			runsEmpty: "本会话还没有派发过子智能体。",
			status: {
				queued: "排队中",
				running: "运行中",
				needs_reply: "需要回复",
			...
			queueReason: "等待运行槽（第 {n} 位）",
			waitingApprovalReason: "等待你确认（审批坞在输入框）",
			abortedBy: "由用户中止",
			actions: {
				abort: "中止",
				cancel: "取消",
			...
			error: "错误",
			citeHeader: "子智能体 {agent} 的结果（{time}）：",
			promptDispatch:
				"请调用 subagent 工具派发子智能体 {agent} 执行以下任务，完成后把结论整合进当前工作：\n{task}",
			promptRequired: "必需工具：{tools}",
		},
	},
	process: {
		label: "过程",
	...
	message: {
		...
		subagent: {
			...
			queued: "排队中",
			queuedReason: "等待运行槽（第 {n} 位）",
			aborted: "已中止",
			waitingApproval: "等待审批",
			error: "错误",
			panelDispatched: "你从面板派发了 {agents}（{n} 个任务）",
			result: "子智能体结果 · {agent}",
			cite: "引用到输入框",
			expandRecord: "展开记录",
			collapseRecord: "收起记录",
			followUpDelivered: "已作为 followUp 交给主模型",
			followUpPending: "等待进入上下文 · 主模型正在生成",
			noFollowUp: "未交给主模型 · 可引用",
	...
	settings: {
		...
		permissions: {
			title: "权限规则",
			file: "规则文件",
			effectHint: "保存后下一次工具调用即生效，无需重启",
			missingFile: "文件尚不存在：当前按默认规则运行，保存后创建。",
			parseError: "文件无法解析（{error}）：当前按默认规则运行，保存会覆盖损坏内容（旧文件保留为 .bak）。",
			disabledBanner: "文件中 enabled=false：权限门控已关闭。此开关只能手改文件恢复，本页不会改动它。",
			loading: "正在读取规则文件…",
			loadError: "读取失败：{error}",
			retry: "重试",
			boundary: "工作区边界",
			outsideRead: "工作区外读取",
			outsideReadHint: "read / ls / show_image 的目标落在全部工作区根之外时",
			outsideWrite: "工作区外写入",
			outsideWriteHint: "edit / write 的目标落在全部工作区根之外时",
			temporary: "系统临时区",
			temporaryHint: "tmpdir ∪ /tmp 内的路径与删除目标：可放松 ask，永不放松 deny",
			autoApprove: "项目内编辑自动放行",
			autoApproveHint: "关闭后 edit / write 即使在项目内也走审批坞",
			rules: "工具规则",
			fallback: "*（未列出的工具）",
			fallbackHint: "没有专门规则的工具一律按此动作",
			patternTable: "模式表",
			orderHint: "键序即评估序，后命中生效",
			colIndex: "#",
			colPattern: "模式",
			colAction: "动作",
			moveUp: "上移",
			...
			removeRow: "删除",
			locked: "自保护",
			lockedHint: "自保护规则：不能设为 allow、不能删除、固定在表尾",
			addPattern: "添加模式",
			patternPlaceholder: "如 git push* 或 *.env",
			removeTool: "移除该工具规则",
			addTool: "添加工具规则",
			addToolCustom: "自定义工具名…",
			customToolPlaceholder: "工具名（如 my_tool）",
			add: "添加",
			probe: "试算",
			probePlaceholder: "输入一条命令，如 git push --force origin main",
			probeRun: "试算",
			probeHit: "命中 #{index} {pattern} → {action}",
			probeFallback: "无模式命中 → {action}（工具级 / 全局兜底）",
			probeSegment: "最严段：{segment}",
			probeNote: "按当前草稿只跑规则链；工作区边界、临时区与项目内自动放行取决于会话目录，此处不模拟。",
			advanced: "高级",
			audit: "审计日志",
			auditHint: "permission-audit.jsonl：fullAccess 档下本会被 ask / deny 的高危操作，最近 20 条",
			auditEmpty: "暂无记录",
			openLocation: "打开文件位置",
			restoreDefaults: "恢复默认",
			restoreConfirm: "恢复为默认规则？当前文件会保留为 permissions.json.bak。",
			discard: "放弃更改",
			save: "保存",
			saving: "保存中…",
			saved: "已保存，下一次工具调用起生效",
			unsaved: "有未保存的更改",
			invalid: "有 {count} 处需要修正才能保存",
			conflictTitle: "文件已在外部修改",
			conflictHint: "保存已暂停。「重新加载」会丢弃这里的更改；「覆盖」会用这里的内容替换磁盘上的文件。",
			conflictReload: "重新加载",
			conflictOverwrite: "覆盖",
			issue: {
				action: "动作只能是 allow / ask / deny",
				"numeric-key": "纯数字键会被引擎重排，请改名",
				"empty-pattern": "模式或工具名不能为空",
				"self-protection": "自保护规则必须存在、不能为 allow，且不能被其后的 allow 覆盖",
				shape: "重复的工具名或模式",
			},
			action: {
				allow: "allow · 放行",

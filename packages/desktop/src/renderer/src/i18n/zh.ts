...
export const zh = {
	...
	taskConsent: {
		...
		title: "确认一次，交给它做完",
		active: "已确认 · 自动做到交付",
		limits: "这个任务最多自动执行 {calls} 步、意外中断后最多自动恢复 3 次，到头就停下来找你。",
		directories: "会写入这些文件夹（包括子文件夹）：",
		noDirectories: "不新增可写文件夹；改动文件仍按你现有的权限设置逐项确认。",
		boundary:
			"确认后它会按下面的计划自己做完。要是执行中还弹出命令确认，选「本次任务全部允许」就不会再逐条问了。删除文件、碰敏感内容、做计划外的事，都还是会先征得你同意；你随时可以停。",
		approve: "确认一次，执行到交付",
	},
	common: {
		save: "保存",
	...
	skillsCatalog: {
		...
		aliases: "兼容名称：",
		showSpecialized: "显示专用初始化与内部参考（{count}）",
		hideSpecialized: "收起专用初始化与内部参考",
	...
	workbench: {
		...
		sshSummary: "Agent 可以发起 SSH 尝试，但读取本机密钥前会暂停等待确认。",
		localOpenSsh: "仅本机 OpenSSH 密钥",
		reviewGuard: "查看 SSH 守卫设置",
		online: "工具在线",
		ready: "已就绪",
		model: "模型",
		tools: "工具",
		modelTools: "模型与工具",
		controlTitle: "Agent 为你工作",
		controlHint: "关键操作会停下来请求确认；来源、文件和密钥访问都保持可见。",
		task: {
			title: "任务工作台",
			count: "项任务",
		},
	...
	composer: {
		...
		permissionDefaultDesc: "写文件等敏感工具需在审批坞确认；读/列目录等仍自动执行",

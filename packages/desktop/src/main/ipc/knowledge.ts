...
export function registerKnowledgeIpc(backend: PiBackend): void {
 ...
 handle(IpcChannels.KnowledgeOpen, async (input: Parameters<KnowledgeApi["openKnowledgeTarget"]>[0]) => {
  ...
  if (target.kind === "note") await shell.openExternal(`obsidian://open?path=${encodeURIComponent(target.path)}`);
  else { const error = await shell.openPath(target.path); if (error) throw new Error(error); }
 });

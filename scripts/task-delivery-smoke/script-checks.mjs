...
export async function verifyScriptReaders({ js, wait, cwd, checks }) {
	const fixtures = [
		...
		[
			...
			"Python 大写",
			"python",
			'open("preview-must-not-execute", "w").write("must not run")\nprint(1)\n',
		],
	...
	for (const [name, label, language, source] of fixtures) {
		...
		assert(
			(await js("document.querySelector('[data-testid=resource-code] pre').innerText")).includes(
				source.split("\n")[0],
			),
		);
		checks.push(
			`${name}: actual assistant Markdown link → production preload/IPC → bounded source with loaded ${language} syntax highlighting`,
		);
	}

...
const names = [
	...WORKFLOW_PROFILES.filter((p) => !excluded.has(p.name)).map((p) => p.name),
	"setup-pre-commit",
	"user-local-tool",
];
...
(window as any).__executions = 0;
(window as any).pi = {
	listSlashCommands: async (id: string) =>
		id === "empty"
			? []
			: id === "missing-review"
...
function Fixture() {
	...
	const [sessionId, setSessionId] = useState("fixture");
	...
	return (
		<div className="mx-auto max-w-[900px] space-y-4 p-5">
			...
			<button
				...
				id="missing"
				onClick={() => {
					setSessionId("missing-review");
					setSelected(null);
					setText("");
				}}
			>
				Missing blind reviewer
			</button>
			<button
				type="button"
				id="empty"
...
if (!rootEl) throw new Error("workflow-catalog-smoke: #root missing");

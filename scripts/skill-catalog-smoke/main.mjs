...
async function run() {
	try {
		...
		assert.equal(await js("document.querySelectorAll('#skill-panel li').length"), 7);
		checks.push("specialized setup and support collapsed in settings");
		...
		assert.equal(await js("document.querySelector('[data-command=\"skill:setup-pre-commit\"]')"), null);
		checks.push("one default Obsidian setup; no duplicate aliases or specialist setup");
		await js(
			"[...document.querySelectorAll('#command-menu button')].find(b=>b.innerText.includes('显示专用')).click()",
		);
		await wait("!!document.querySelector('[data-command=\"skill:setup-pre-commit\"]')");
		checks.push("explicit reveal retains specialist workflows");
		...
		checks.push("setup search yields one canonical action plus three clearly separate tools");
		await mkdir(join(root, "screenshots"));
		await writeFile(
			join(root, "screenshots/01-setup-menu.png"),
			(await window.webContents.capturePage()).toPNG(),
		);
		await js("document.querySelector('#command-input').focus()");
		for (const key of ["Down", "Down", "Tab"]) {
		...
		await wait("document.querySelector('#selected-command').textContent==='skill:setup-pre-commit'");
		checks.push("actual keyboard handler selects the same row that the menu highlights");
		...
		checks.push("Chinese purpose search expands the correct group");
		await writeFile(
			join(root, "screenshots/02-skill-search.png"),
			(await window.webContents.capturePage()).toPNG(),
		);
		await type("#skill-panel input", "");
		await js(
			"document.querySelector('#lang').click();document.documentElement.setAttribute('data-theme','dark')",
		);
		await wait("document.querySelector('#skill-panel').innerText.includes('Tool-specific setup')");
		window.setSize(640, 900);
		await new Promise((r) => setTimeout(r, 100));
		assert(await js("document.documentElement.scrollWidth<=innerWidth"));
		checks.push("English and dark narrow layout without horizontal overflow");
		await writeFile(
			join(root, "screenshots/03-dark-narrow.png"),
			(await window.webContents.capturePage()).toPNG(),
		);

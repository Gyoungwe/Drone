...
try {
	for (const count of sizes) {
		...
		try {
			await mkdir(cwd);
			await configureObsidian({ cwd, vault });
			let totalBytes = 0;
			const paths = [];
			for (let base = 0; base < count; base += 40) {
				await Promise.all(
					Array.from({ length: Math.min(40, count - base) }, async (_, j) => {
						const i = base + j,
							body =
						...
						const path = `Library/Papers/note-${String(i).padStart(6, "0")}.md`;
						paths.push(path);
			...
			const indexed = await service.request("reconcile");
			const initialMs = performance.now() - begin;
			...
			for (let i = 0; i < 30; i++) {
				...
				const result = await service.search(prep.ticket, cwd, {
					query: `marker${Math.floor((i * count) / 30)}`,
					limit: 5,
				});
			...
			const readBefore = after.stats.bodyReads;
			begin = performance.now();
			await writeFile(join(vault, paths[0]), "# Incremental update\nuniquedeltatoken\n");
			await service.request("changed", { paths: [paths[0]] });
			const deltaMs = performance.now() - begin;
			const delta = await service.request("status");
			const found = await service.search(prep.ticket, cwd, { query: "uniquedeltatoken" });
			...
			while (stable < 2 && performance.now() - settleStarted < 5000) {
				...
				stable =
					status.coverage === "ready" && status.pendingChanges === 0 && status.problems.length === 0
						? stable + 1
						: 0;
			}
			assert.equal(stable, 2, "Synthetic index did not reach a stable publication precondition");
			const refreshed = await service.search(prep.ticket, cwd, { query: "uniquedeltatoken" });
			...
			for (let i = 0; i < 30; i++) {
				...
				const proof = await service.validateAnswer(
					prep.ticket,
					cwd,
					`Fixture statement. [[${found.hits[0].path.slice(0, -3)}]]`,
				);
				assert.equal(proof.status, "ready");
				publication.push(performance.now() - start);
			}
			const publicationAfter = await service.request("status");
			publication.sort((a, b) => a - b);
			latencies.sort((a, b) => a - b);
			const row = {
				notes: count,
				syntheticMarkdownBytes: totalBytes,
				indexedNotes: indexed.noteCount,
				initialIndexMs: +initialMs.toFixed(2),
				warmSamples: latencies.length,
				warmMedianMs: +latencies[Math.floor(latencies.length / 2)].toFixed(2),
				warmP95Ms: +latencies[Math.ceil(latencies.length * 0.95) - 1].toFixed(2),
				hotBodyReads: after.stats.bodyReads - before.stats.bodyReads,
				hotFullReconciliations: after.stats.reconciliations - before.stats.reconciliations,
				oneNoteUpdateMs: +deltaMs.toFixed(2),
				oneNoteBodyReads: delta.stats.bodyReads - readBefore,
				publicationSamples: 30,
				publicationMedianMs: +publication[15].toFixed(2),
				publicationP95Ms: +publication[28].toFixed(2),
				publicationBodyReads: publicationAfter.stats.bodyReads - publicationBefore.stats.bodyReads,
				publicationFullReconciliations:
					publicationAfter.stats.reconciliations - publicationBefore.stats.reconciliations,
			};
			rows.push(row);
			console.error("Completed synthetic fixture:", count);
		} finally {
...
console.log(
	JSON.stringify(
		{
			kind: "synthetic-local-only",
			node: process.version,
			platform: platform(),
			arch: arch(),
			measuredAt: new Date().toISOString(),
			includes:
				"local index search/navigation revalidation and stable-index one-source publication validation; excludes LLM, network and file-watcher settling time",
			rows,
		},
		null,
		2,

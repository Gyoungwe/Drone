...
const repo = resolve("."),
	root = await realpath(await mkdtemp(join(tmpdir(), "drone-workflow-catalog-ui-")));
await mkdir(join(root, "profile"));
console.log("Isolated skill UI fixture:", root);
await writeFile(
	join(root, "entry.tsx"),
	`import ${JSON.stringify(join(repo, "scripts/workflow-catalog-smoke/renderer.tsx"))};import './style.css';`,
);
await writeFile(
	join(root, "style.css"),
...
await copyFile(join(repo, "scripts/workflow-catalog-smoke/main.mjs"), join(root, "main.mjs"));

...
| 子智能体面板（右栏「子智能体」页签：可用 / 派发 / 本会话运行） | backend `tools/subagent/panel.ts`（`SubagentPanelService`）+ `pi-backend.ts`（`listSessionSubagents/dispatchSubagents/abortSubagentRun/listSubagentRuns`）；IPC `subagents:*`（`main/ipc/subagents.ts`）；事件 `subagent_run`（reducer 原地更新派发行 + `stores/subagents.ts`）；UI `panel/SubagentsPane.tsx` + `subagents-form.ts`、`chat/SubagentAvatar.tsx`（djb2 定色 + data-state 表情）、`chat/SubagentResultCard.tsx`、审批坞来源胶囊；文档 [subagents-panel.md](subagents-panel.md)；冒烟 `scripts/check-subagents-ui.mjs` |
